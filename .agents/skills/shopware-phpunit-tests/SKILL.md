...
- `@codeCoverageIgnore` is for pass-through code only. A PHPStan rule (`CodeCoverageIgnoreEvaluationRule`) fails on annotated methods that branch, throw, mutate a value (`unset`, `+=`, `.=`, `++`, a second write to a local) or call `$this->…()`/`self::…()` for its effect (access guards, hooks). Write a unit test for such a class instead, or point a `@see` at a dedicated integration or devops test.
