...
- Use only the integration-test traits the scenario needs. For repository tests that only require the service container and transaction rollback, use `KernelTestBehaviour` and `DatabaseTransactionBehaviour` instead of the broad `IntegrationTestBehaviour` bundle.
...
- When a test explicitly verifies that a behavior throws while a major feature flag is active, mark that test method `@deprecated tag:vX.Y.0` and state that it is removed with the corresponding feature flag. This makes temporary major-mode assertions discoverable and removable with the deprecated behavior.
- In unit tests, current major feature flags are active by default. Test legacy/off behavior by disabling the flag with the `#[DisabledFeatures]` attribute; do not use `Feature::fake()` just to activate the current major flag. While two majors are in flight the unit bootstrap activates both, so a test asserting the older major's behaviour has to disable the newer one (`#[DisabledFeatures(['v6.9.0.0'])]`); an integration test pins itself with `Feature::skipTestIfActive('v6.9.0.0', $this)` instead.
- In integration tests, feature-flag state comes from the job configuration (the default integration job runs with flags off, integration-major once per in-flight major with `FEATURE_ALL=<major>`, e.g. `v6.8.0.0`), and the suite may run multiple times with flags on and off. `#[DisabledFeatures]` has no effect there and the test runner rejects it — a test carrying the attribute fails the run. Skip tests explicitly with `Feature::skipTestIfActive()` or `Feature::skipTestIfInActive()` when the current feature-flag value is not the one the scenario expects.
...
- Do not compose helpers into a unit test class through a trait; use a static fixture class or a stub under `Stub/`. `ClockSensitiveTrait` is the only allowed trait; set environment variables with `TestEnvironment::set()` (restored automatically after each unit test) instead of composing `EnvTestBehaviour`, and register listeners directly on the dispatcher the test owns instead of composing `EventDispatcherBehaviour`, a Danger rule fails new unit test files with any other.
