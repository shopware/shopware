...
const localUrl = (raw) => {
  ...
  if (/^https?:\/\//i.test(target)) {
    try { return ['localhost', '127.0.0.1', 'host.docker.internal'].includes(new URL(target).hostname) ? target : null; }
    catch { return null; }
  }
...
export async function check({ plan = readJson(FILES.plan, {}) } = {}) {
  if (!appUrl() || plan.executor !== 'playwright') return { ok: true, skipped: true };
  const checks = readinessChecks(plan).filter((c) => c && (c.kind ?? 'browser') === 'browser');
  if (checks.length === 0) { writeReport({ ok: true, skipped: true, reason: 'no browser readiness checks' }); return { ok: true, skipped: true }; }
...
async function runCheck(page, c, index, failures, observations) {
  ...
  if ((rect?.width ?? 0) < Number(c.min_width ?? 1)) failures.push(`${name}: ${selector} too narrow (${Math.round(rect?.width ?? 0)}px)`);
  if ((rect?.height ?? 0) < Number(c.min_height ?? 1)) failures.push(`${name}: ${selector} too short (${Math.round(rect?.height ?? 0)}px)`);
...
async function storageStateOptions(plan) {
  ...
  if (plan.layer === 'storefront-ui' && plan.browser_state?.auto_cookie_consent !== false) {
    ...
    if (res.status === 0 && fs.existsSync(state)) return { storageState: state };
  }
  return {};
}
