 ...
 */
export async function schemaCommand(entity) {
  const data = await schema(entity);
  if (!entity) {
    console.log(Object.keys(data).sort().join('\n'));
    return;
  }
  const def = data[entity];
  if (!def) {
...
export async function searchCommand(entity, criteriaJson) {
  ...
  const result = await search(entity, criteria);
