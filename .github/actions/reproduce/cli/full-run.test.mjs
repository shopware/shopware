...
const SRC = pathToFileURL(path.join(HERE, 'full-run.mjs')).href;
 ...
 */
function runFull(cwd, { target = 'reported', out = 'result.json', reset = false, env = {} } = {}) {
  const code = [
...
const readResult = (cwd, name = 'result.json') => JSON.parse(fs.readFileSync(path.join(cwd, name), 'utf8'));
const writePlan = (cwd, plan) => fs.writeFileSync(path.join(cwd, 'reproduction-plan.json'), JSON.stringify(plan));
