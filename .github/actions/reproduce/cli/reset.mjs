...
function parseDatabaseUrl(url) {
  const u = new URL(url);
  return { host: u.hostname || '127.0.0.1', port: u.port || '3306', user: decodeURIComponent(u.username) || 'root', pass: decodeURIComponent(u.password), name: u.pathname.replace(/^\//, '') };
}

export function reset() {
  if (!fs.existsSync(FILES.snapshot)) { console.log('no DB snapshot — running on the current state'); return; }
  if (!process.env.DATABASE_URL) { console.warn('::warning::DATABASE_URL not set — skipping DB reset'); return; }

  const db = parseDatabaseUrl(process.env.DATABASE_URL);
  console.log('resetting DB to the clean snapshot…');
  const restore = spawnSync('bash', ['-c', `gunzip -c "${FILES.snapshot}" | mysql -h"${db.host}" -P"${db.port}" -u"${db.user}" "${db.name}"`],
