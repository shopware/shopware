...
| 2 | Shop URL | `APP_URL=http://localhost:8000` (Symfony server started in [finish-provision.sh:31](../steps/finish-provision.sh)) exported via `GITHUB_ENV` | `localhost:8000` is dead: wrong host (container) AND port 8000 is not in the awf host-port allowlist. **CONFIRMED by probe run 28922261954**: of awf's default `allow-host-ports 80,443,8080`, **8080 is claimed by gh-aw's own MCP Gateway** (`MCP_GATEWAY_PORT=8080`; binding it collides and kills the agent step), and 80/443 need privilege/TLS. So there is NO free default-allowed host port for the shop. | Add a **custom allowed host port** for the shop — this is unavoidable, not optional. Check whether gh-aw v0.81.2 frontmatter exposes an allow-host-ports field; if it does, use it and bind the shop there (e.g. keep 8000 and declare it). If it does not, the June `compile.sh` perl patch that appended a custom port (18080) to `--allow-host-ports` is the *supported seam*, not a workaround to avoid — reinstate it as a documented compile step. Then export the agent-facing `APP_URL=http://host.docker.internal:<port>`; post-steps keep `steps.provision.outputs.app_url` (localhost). |
