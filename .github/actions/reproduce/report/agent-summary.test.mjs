...
const SCRIPT = path.join(path.dirname(fileURLToPath(import.meta.url)), 'agent-summary.mjs');
...
const logLines = (...objs) => objs.map((o) => (typeof o === 'string' ? o : JSON.stringify(o))).join('\n');
const writeLog = (cwd, content) => {
  const p = path.join(cwd, 'agent.log');
  fs.writeFileSync(p, content);
  return p;
};
const result = (text) => ({ type: 'result', result: text });
