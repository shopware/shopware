...
| `sw-triage` | `workflow_dispatch` (input: `issue_number`), `/sw-triage` issue comment, `qi/sw-triage` issue label | `claude` / `claude-sonnet-4-6` | `triage-output.json` via `upload-artifact` |
| `sw-review` | `qi/sw-review` PR label, `workflow_dispatch` (input: `pr_number`), `/sw-review` on PRs | `claude` / `claude-sonnet-4-6` (security & architecture personas → `claude-opus-4-8`) | inline findings via `create-pull-request-review-comment` + summary via `submit-pull-request-review` |
...
The Review agent is read-only and never approves. It orchestrates one **inline sub-agent per persona** (the `## agent:` blocks in `sw-review.md`; `security` and `architecture` escalate to `claude-opus-4-8`), merges the per-persona findings, and publishes them as inline PR review comments plus a single summary review (`COMMENT` or `REQUEST_CHANGES`). The rubric and personas are shared with the interactive `sw-review` skill via `.github/aw/shared/sw-review-policy.md` and `.agents/skills/sw-review/{personas,references}/`. Note: inline sub-agent dispatch (and whether sub-agents inherit the expected read-only tool scope) should be verified on the first dispatch — see the registration trick below to test on a feature branch before merging to trunk.
