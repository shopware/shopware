...
current_major_alias() {
  ...
  curl -fsSL "https://releases.shopware.com/changelog/index.json" | jq -r '[.[] | select(test("[a-zA-Z]") | not)] | first | split(".") | [.[0], .[1], "x-dev"] | join(".")'
}
...
custom_version_extensions() {
  ...
  git clone --depth=1 "https://gitlab-ci-token:${GITLAB_SAAS_TOKEN}@gitlab.shopware.com/shopware/6/product/saas.git" "${tmpdir}"
...
commit_date() {
  ...
  gh api "/repos/shopware/${repo}/branches/${branch}" | jq -r '.commit.commit.committer.date' | xargs -I '{}' date -R --date="{}"
}
...
<ul>
<li><span>shopware/platform: <b>$(commit_date "shopware-private" "${deployment_branch_name_url_encoded}")</b></span></li>
<li><span>shopware/commercial: <b>$(commit_date "SwagCommercial" "${deployment_branch_name_url_encoded}")</b></span></li>
