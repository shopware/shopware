...
safe-outputs:
  ...
  upload-artifact:
    max-uploads: 4             # analysis.json + fixtures.json + the executor script (+headroom);
                               # max-uploads:1 silently dropped the script on direct/playwright plans
...
files. HARD BUDGET: write a best-effort `analysis.json` within your first ~6 tool calls; refine
only if turns remain; NEVER finish without it on disk (a lower `confidence` is the escape
valve, not more exploration).
