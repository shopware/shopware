...
- Product comparison exports: the shipped Google, Idealo and Billiger starter templates now use `product.stock`, and a migration renames `product.availableStock` to `product.stock` in the header, body and footer of every stored export template — templates you edited yourself included. Nothing else about them is changed, and because both accessors carry the same value the rendered feed is identical. An accessor reached through an alias (`{% set p = product %}{{ p.availableStock }}`) cannot be recognised and is left alone; adjust those manually before 6.8.
