...
The empty states of Extensions > My extensions and the extension store landing page now render `mt-empty-state` instead of a `sw-meteor-card` with custom markup and an illustration.

The blocks that wrapped the previous markup still exist but are empty and deprecated for removal in v6.9.0, because their content became `mt-empty-state` props. The `assetFilter` computed of `sw-extension-my-extensions-listing` and `sw-extension-store-landing-page` is deprecated for the same reason and is still available for overrides that render their own image.

The following snippets have been removed:
...
The landing page copy now uses the new snippets `sw-extension-store.landing-page.activationHeadline` and `.activationDescription`. Shops that overrode the removed keys in the database need to move their text to the new keys.

The class `.sw-extension-store-landing-page__wrapper-label` no longer exists, and `.sw-extension-store-landing-page__wrapper` no longer carries a background, border, fixed width or fixed height.
