...
- The record is pseudonymous. `consentId` is an opaque random token the storefront generates on the first decision and keeps in the `cookie-consent-id` cookie (lifetime equals `shopware.cookie_consent.retention_days`, refreshed with every decision), so later decisions of the same visitor share it. No IP address, user agent, session id or customer id is stored. Because the token links records to a browser, the log is personal data and falls under access and erasure requests; a data subject retrieves their records by providing the token from their browser.
