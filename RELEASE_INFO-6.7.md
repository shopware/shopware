...
### `product-export:generate` no longer skips scheduler-managed exports silently

**What changed:** `bin/console product-export:generate` previously skipped product exports that are configured to be generated by the scheduler (`generateByCronjob`) without any feedback, so the command exited successfully even though no file was written for those exports. It now logs a warning (using the new `Shopware\Core\Content\ProductExport\ProductExportException::generatedByScheduler()` message, error code `PRODUCT_EXPORT__GENERATED_BY_SCHEDULER`) for each skipped export and continues generating the remaining ones.

**Why:** A run that produced no output was indistinguishable from a successful run, so operators could not tell whether an export was intentionally left to the scheduler or whether something had failed. The warning makes the skip explicit.

**Who needs to care and when:** Anyone running `product-export:generate` (manually or from automation) for a sales channel that contains at least one export with "Generate via scheduler" enabled. Such runs now emit a warning per skipped export on the console and via the application log.

**How to adjust:** No action is required — the other exports are still generated and the command still exits successfully. Pass `--force` (`ExportBehavior::ignoreCache`) to regenerate a scheduler-managed export from the CLI anyway; this also suppresses the warning (see the related change for issue #18498). The behaviour is limited to the console command: the `ProductExporter` service, the storefront export route and the scheduled task are unchanged.
