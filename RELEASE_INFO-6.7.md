...
These `.vue` files are now linted with the same type-aware rules as `.ts`: `composer admin:check-extensions` reports calls to `@deprecated` members and unhandled (floating) promises, and flags unused `<script setup>` bindings while leaving bindings used only in the template alone. The `@typescript-eslint/no-unsafe-*` rules stay off on `.vue` (the SFC type information available to ESLint is incomplete, matching the `@vue/eslint-config-typescript` default); `vue-tsc` remains the type checker for `.vue`.
