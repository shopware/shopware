...
- Logging can be switched off with the `core.cookieConsent.logEnabled` system config (default: enabled), in the admin under Settings > Basic information, in the new "Cookie consent logging" card. Turn it off when a third-party consent manager keeps the evidence instead. Decisions that were already recorded are kept and still expire through the retention period. While it is off the routes still answer `204`, and the storefront stops sending the beacon at all.
- The new daily scheduled task `cookie_consent_log.cleanup` deletes log entries older than the retention period, configurable via the `core.cookieConsent.logRetentionDays` system config (default: 120 days, `0` deletes immediately, negative values disable the cleanup), and removes banner snapshots that are no longer referenced. Operators can manage the retention period in the admin under Settings > Basic information, in the new "Cookie consent logging" card. The value applies to all sales channels.
