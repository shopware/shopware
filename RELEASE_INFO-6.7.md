...
`Settings > Login & Registration` gains `showNameFieldsForCompanyAccounts` and `nameFieldsRequiredForCompanyAccounts`. Both default to on, for new and for upgraded installations, so nothing changes until a shop turns one of them off. Together they cover the three states a contact person can have on a commercial account: required, optional and hidden. A hidden field is never submitted and therefore never required, the same rule the phone number and birthday pairs follow. Once the contact person is no longer mandatory the company name takes its place and becomes required.

The account type of a write comes from the request first and from the authenticated customer second. A registration without an account type is a private one and keeps the names required, and a logged in commercial customer stays one even while the account type selection is switched off. The rule lives in the `CompanyAccountNameFields` service: `areVisible()`, `areRequired()` and `areOptional()` answer for a sales channel, `relax()` drops the blank check of a `DataValidationDefinition` and requires the company where the caller asks for it, and `normalize()` turns an absent name into the empty string the data abstraction layer accepts.

The first and last name fields of `customer`, `customer_address`, `order_customer` and `order_address` now carry the `AllowEmptyString` flag. The columns stay `NOT NULL` and the getters keep returning `string`, but an empty string is accepted on every write path, including the Admin API. A `PreWriteValidationEvent` subscriber keeps one rule on all of them: a customer or an address may not end up naming nobody, so both names may only be empty for a commercial account with a company or an address that carries one. A private account still needs a contact person everywhere.

The `customer` and the `order_customer` entity gain a runtime field `displayName`, holding the person name, or the company name when there is no contact person. Use it instead of joining `firstName` and `lastName` when you render a customer:
...
`CustomerEntity::getDisplayName()` resolves the name from the live `firstName`, `lastName`, `company` and `accountType`, so a private account without a name renders nothing rather than its company. `OrderCustomerEntity::getDisplayName()` resolves it from the names and the company of the snapshot, which has no account type. A subscriber per entity fills the runtime field on the `loaded` and `partial_loaded` events, which is what carries the value into the API responses, and both getters return that assigned value first, so an extension can replace it on load. `CustomerEntity::__toString()` returns the display name too. Being a runtime field it cannot be sorted or filtered in a `Criteria`; sort on `lastName` or `company` instead.
...
In the Administration the company becomes an account level field of a commercial customer, and a migration copies it from the default billing address for accounts that were created without one. The new `companyAccountNameFieldsService` answers whether a contact person is required for a sales channel, and the customer list, the order list and every other place that names a customer read `displayName` from the API.
...
The buyer name on invoices no longer repeats the company name when the person name and the company name are identical, and no longer starts with a `-` when only a company name is present. The ZUGFeRD renderer and the document v2 trade party view both take the name from `OrderCustomerEntity::getBuyerName()` now, so they cannot drift apart again.
