...
Rejections surface in your editor as well as in the build: the `valid-shopware-setup` ESLint rule runs the same validation. Editor support (the macro globals, the native-setup lint rules and the macro types) comes from the extension tooling — run `composer admin:setup-extension-tooling` (or `bin/console administration:setup-extension-tooling` in a Composer install) and it is generated for you; there is nothing to copy by hand. Full authoring reference: `src/Administration/Resources/app/administration/technical-docs/03-extensibility/07-native-setup-authoring.md`.
...
- **Native-setup authoring is supported out of the box.** The generated ESLint config declares the compile-time macro globals (`swDefinePublic`, `swDefineOverride`, `useSwPreviousState`, `useSwProps`, `useSwContext`) and enables the two native-setup guards (`sw-core-rules/valid-shopware-setup`, `sw-core-rules/native-setup-filename`), and the shared type surface carries the macro declarations so they type-check. A native-setup plugin needs no hand-copied ESLint/TypeScript template — and because everything resolves through the same package resolution as the rest of the preset, a dependency bump cannot leave a stale copied path behind.
