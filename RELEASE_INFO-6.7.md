...
- Every "accept all", "accept required only" and custom-selection interaction with the cookie banner is sent fire-and-forget (`navigator.sendBeacon`) to the new storefront route `POST /cookie/consent-log`, which proxies to the new Store API route `POST /store-api/cookie-consent-log`. Headless frontends call the Store API route directly.
- The client only reports raw facts: its `consentId`, the action and the names of the cookies the visitor ticked (`acceptedCookies`). The per-group verdict is derived server-side against the configuration the server holds, so it cannot be shaped by the client. `groupDecisions` distinguishes `accepted`, `partial` and `rejected` per cookie group; a group counts as `accepted` only when every cookie the visitor could tick in it was accepted.
- The record is pseudonymous. `consentId` is an opaque random token the storefront generates on the first decision and keeps in the `cookie-consent-id` cookie (30 days, refreshed with every decision), so later decisions of the same visitor share it. No IP address, user agent, session id or customer id is stored. Because the token links records to a browser, the log is personal data and falls under access and erasure requests; a data subject retrieves their records by providing the token from their browser.
- While recording is on, `cookie-consent-id` is declared as a technically required cookie (`cookie.groupRequiredConsentId`) and listed visibly in the cookie banner, so visitors can find the token. Add it to your privacy policy. Switching recording on or off changes the cookie configuration, so the banner is shown once more to visitors who already decided. While recording is off, the storefront sends no beacon and sets no `cookie-consent-id` cookie.
- Withdrawals are recorded like consents: re-opening the banner and removing a category writes a second record with the narrower `groupDecisions` and the same `consentId`, giving a per-visitor timeline.
- A snapshot of the cookie banner configuration is stored once per configuration hash, preserving what the banner said when consent was given. Every record references its snapshot via `configHash`. Snapshots contain no personal data and are kept.
- New CLI commands read the log: `bin/console cookie:consent:show <consent-id>` prints the decisions and banner configurations recorded for one visitor, `bin/console cookie:consent:export [--from] [--to] [--sales-channel] [--format=json|csv]` streams the log for compliance exports.
- Where the log is kept is pluggable. `Shopware\Core\Content\Cookie\ConsentLog\AbstractCookieConsentLogStorage` defines the contract, `shopware.cookie_consent.log_storage` selects the implementation: `none` (default) discards every decision, `database` writes to the new `cookie_consent_log` and `cookie_consent_config_snapshot` tables, `filesystem` writes one JSON file per decision to `shopware.filesystem.private` below `shopware.cookie_consent.filesystem_path` (default `cookie-consent`), in `<year>/<month>/<day>/<hour>/` directories with the consent id in the file name, plus one `snapshots/<hash>.json` per banner configuration. Point the private filesystem at object storage to keep a high-volume log out of the shop database, or register a custom service tagged `shopware.cookie_consent.log_storage`. The switch is per installation, not per sales channel.
- With the `filesystem` storage, a lookup by consent id lists every hour directory of the archive, so on a large archive `cookie:consent:show` takes minutes rather than seconds. Multi-server setups need a shared filesystem or object storage. Each decision is one small file, plan inodes and backups accordingly.
- The daily scheduled task `cookie_consent_log.cleanup` deletes records older than `shopware.cookie_consent.retention_days` (default: 120 days). The `filesystem` storage deletes whole hour directories, so its retention is accurate to one hour. Evidence retention is a merchant decision: regulators can challenge processing for years, so consider a longer period and the storage it needs.
- Records carry a `source` (`banner`) so decisions collected through other channels, e.g. browser-level consent signals, can share the schema later.
- The new PHP event `Shopware\Core\Content\Cookie\Event\CookieConsentLoggedEvent` is dispatched for every stored decision and carries the `CookieConsentRecord`. It is not a webhook event.
- Both routes are rate limited per client IP (`cookie_consent_log`, 60 requests per 60 seconds, `sliding_window`). The IP is only the limiter key and is never stored.
