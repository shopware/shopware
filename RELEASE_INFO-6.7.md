...
- `composer admin:setup-extension-tooling` discovers all installed extensions with Administration sources (warning with the `bundle:dump` command when `var/plugins.json` is stale) and generates disposable, marker-owned configs (one runtime and spec tsconfig per canonical Administration source target under `var/admin-extension-tooling/`, root `tsconfig.json` / `eslint.config.mjs` projections, IDE bootstraps for VS Code and Zed, a managed ignore block in the project `.gitignore`). Multi-bundle packages remain one reporting/baseline owner while every Administration root is retained in the manifest; `--explain` shows the effective TypeScript and ESLint config for each target. A zero-config plugin gets full typed autocomplete — including installation-specific entity types — and lint coverage without any files of its own. A plugin is included with one command, `--shim=<name>`, which writes a self-ignoring `.shopware-admin/` bridge (holding the machine-specific paths, merging plugin aliases from a committed `tsconfig.aliases.json`) and, for a plugin with no config yet, two small committable `tsconfig.json` / `eslint.config.mjs` that just extend it — visible, committable, and freely customizable as long as the `extends` stays; existing configs are never overwritten. For a multi-bundle package whose single package-level config already governs several Administration roots, `--shim=<name>` auto-detects that config and generates one bridge beside it instead of one per root (or name the directory explicitly with `--root-config=<dir>`); a layout with two or more competing shared configs is refused until you pick one, and genuinely independent per-root configs keep their own per-root bridge. The bridge merges aliases declared beside every covered root without replacing the preset's host paths. The report classifies extensions with one consistent vocabulary — `ready` (zero-config), `bridged` (own config composes the preset), `bridge-unwired` (bridge exists but a config has not composed it yet), and `needs bridge` (ships its own config that does not compose yet) — lists changed files by path, distinguishes git-ignored bridge files from committable plugin-owned files (including in the `--check` dry-run), and prints only the actually-missing next step. Per-tool the same states read as `managed` / `bridged` / `unmanaged`.
- `composer admin:check-extensions` runs every unique `vue-tsc` program and effective ESLint config required by an extension's Administration targets with the Administration's pinned tool versions and readable, colorized output. A shared package-level config is executed once, resolved TypeScript programs are verified to cover every assigned source file, and overlapping diagnostics are de-duplicated before package-level baseline matching. In an interactive terminal it opens a picker to select which extensions to check; `--only=<name[,name]>` or `--all` skip the prompt (non-interactive shells check all). Every `--only` name is resolved independently — a single unknown name fails the whole run and lists the unknown entries, so a renamed or removed target can never leave CI green while it is silently unchecked. Extension configs that do not compose the Shopware preset are visibly skipped with a reason-specific `why:` and the exact `fix:`, never silently green; a skipped writable extension keeps exit 0 with a prominent yellow warning by default (so incremental adoption is possible) and becomes a hard failure under `--fail-on-skipped`, the flag CI should use so `exit 0` cannot mean "checked nothing". The summary line is green for a clean run, yellow when writable extensions were skipped, and red on failure. TypeScript runs are blocked with the fix command while the entity schema is missing; JavaScript-only extensions report a qualified pass instead of a vacuous green; TypeScript and ESLint findings in `vendor/`-installed extensions — including Composer-managed plugins developed under `custom/static-plugins/`, which resolve through `vendor/` and therefore get read-only vendor semantics (no baseline, no `--shim`) — are reported non-fatally by default and become fatal with `--strict-vendor`; develop under `custom/plugins/` for full first-party tooling. `--fix` applies ESLint autofixes including the Shopware `sw-*` → `mt-*` deprecation codemods (not only formatting) and, when findings remain, names the exact follow-up `--update-baseline` command; `--show-commands` prints every underlying program invocation; and `--verbose` shows target-to-config routing. For large plugins, `--summary` adds a triage view that groups findings by rule/code and by file (top-N via `--summary-top`), split into runtime TypeScript, spec TypeScript, ESLint errors, and ESLint warnings with new/baselined/stale counts; `--summary-only` shows just that view for very large logs. A TypeScript diagnostic originating outside the extension root (in the shared type surface) is reported as a tooling/surface failure and can never be recorded as an extension baseline.

Spec files are type-checked too: a dedicated `vue-tsc` program per extension injects jest types (`describe`/`it`/`expect`) and checks only the specs, reported on a `TS (specs)` line, while the runtime program stays free of runner globals. ESLint type-aware-lints specs for zero-config extensions; bridged plugins keep jest-globals linting on specs (still `vue-tsc`-checked).

Because bridging a large plugin — and type-checking its specs — can surface many findings at once, `--update-baseline` records the current findings into a committed, plugin-relative `.shopware-admin-baseline.json` (custom/plugins only). The check then reports `N new · M baselined` and fails only on new findings; stale entries are pruned on the next `--update-baseline`. Matching ignores line and column so a baselined finding survives line drift. Baseline replacement is atomic and is refused when any target stream is unmanaged, blocked, errored, or cannot be parsed with a trustworthy count. `--update-baseline` is mutually exclusive with `--fix`.

Both commands validate their options strictly: `-- --help` prints a generated reference and unknown flags abort with exit 2 before anything is written (note Composer swallows options placed before the `--` separator). Because a swallowed `--check` would otherwise run a real setup, a dedicated `composer admin:setup-extension-tooling:check` alias performs a guaranteed read-only dry-run that cannot mutate files regardless of the separator — the recommended CI drift-gate; `--explain` is likewise read-only.
