...
Store API responses no longer echo the request `sw-context-token` header on cacheable reads. The response header is returned by endpoints that provide or bootstrap shopper state, for example login, logout, registration, password change, guest-order login, context switches, adding cart items, and context gateway login/register commands. Clients should keep using their existing token unless a response explicitly provides a `sw-context-token`.
