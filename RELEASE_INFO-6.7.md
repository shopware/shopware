...
**Run `bin/console es:index` after deploying.** The mapping is added to the live index, but existing documents carry no price until they are reindexed, and a sorting on `product.price` keeps failing with the error above until then. A product that is written in between - after the update, before the reindex - has its price detected as a 32 bit float, because the template that pins it to `double` is not part of the mapping of the old index yet. Elasticsearch keeps a field at the type it was detected as, so that price stays a float until the entity is reindexed: it sorts and filters correctly, but only to about five significant digits, which loses cents above roughly 131,000.
