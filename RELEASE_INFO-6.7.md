...
The built-in cookie banner now records every consent decision server-side, so shop operators can demonstrate that consent was obtained (GDPR Art. 7(1), Recital 42). Recording is active by default. Its routes, database schema, events and PHP classes are experimental until 6.8.0 and not yet covered by the backwards compatibility promise, so they can still change in a patch release.
...
- `cookie-consent-id` is declared as a technically required cookie (`cookie.groupRequiredConsentId`) and is listed visibly in the cookie banner, so visitors can find the token. Add it to your privacy policy. Because the cookie configuration changes, the banner is shown once more to visitors who already decided.
...
- Where the log is kept is pluggable. `Shopware\Core\Content\Cookie\ConsentLog\AbstractCookieConsentLogStorage` defines the contract; the shipped `database` storage writes to the new `cookie_consent_log` and `cookie_consent_config_snapshot` tables, `none` discards every decision. Select it with `shopware.cookie_consent.log_storage` in `shopware.yaml`, or point it at a custom service tagged `shopware.cookie_consent.log_storage` (e.g. a file archive or log management system) to keep a high-volume log out of the primary database. The switch is per installation, not per sales channel.
- The daily scheduled task `cookie_consent_log.cleanup` deletes records older than `shopware.cookie_consent.retention_days` (default: 120 days). Evidence retention is a merchant decision: regulators can challenge processing for years, so consider a longer period and the storage it needs.
