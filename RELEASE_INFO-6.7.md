...
The path is only resolved when a page loads the `categories` and `mainCategories.category` associations. The customer and the guest wishlist page now do, which costs two additional database reads per wishlist page, and a third one when a wishlisted product is only listed through a dynamic product group. Product listings, product sliders, and cross selling do not, so their product boxes report an empty path and their query count is unchanged.
