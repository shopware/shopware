...
Symfony aliases cannot carry tags. To remove a deprecated service alias when a flag becomes active, set a `shopware.inactiveFeature.alias.<alias-id>` container parameter to that flag beside the alias registration. The alias disappears while its target service remains available.
