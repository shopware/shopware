...
Concurrent category writes through the Sync API frequently hit InnoDB deadlocks in the SEO URL and child-count updaters (`1213 Deadlock found when trying to get lock`). Three changes reduce the lock footprint: the `seo_url` table is written with `INSERT ... ON DUPLICATE KEY UPDATE` instead of `REPLACE INTO`, so a colliding row keeps its `id` and `created_at` instead of being deleted and re-inserted; rows whose `is_deleted` flag already holds the target value are no longer rewritten; and child counts are computed with a non-locking read followed by a primary-key-only update. The SEO URL persist transaction additionally runs with `READ COMMITTED` isolation — skipped automatically on connections configured with statement-based binary logging.

Extensions can opt into the same isolation behavior for their own write-only batch transactions with the new `RetryableTransaction::retryableReadCommitted()` helper.
