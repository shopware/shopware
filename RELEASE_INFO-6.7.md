...
- `bin/console cookie:consent:export [--from] [--to] [--sales-channel] [--format=json|csv]` streams the log for compliance exports. To answer a data subject request, filter the export by the `consentId` the visitor provides.
...
- With the `filesystem` storage, multi-server setups need a shared filesystem or object storage. Each decision is one small file, plan inodes and backups accordingly.
