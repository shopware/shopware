...
To avoid this load, we have implemented the configuration `shopware.mail.update_on_order`, which overrides this mechanism. We recommend, to set this configuration as soon as all mail templates in the store are configured correctly. You can simply set this configuration in your `config/packages/*.yaml` file:

```yaml
shopware:
    mail:
        update_on_order: false
```
