...
## Context
The Shopware Language Pack plugin enables us to distribute translations from Crowdin to Shopware installations. While this was a convenient solution in the past, while most of our repositories and workflows were kept private, this approach is unnecessarily convoluted and cumbersome for developers and users in that changes to any snippet require multiple steps:

1. Translations are updated in CrowdIn (by the community or shopware)
2. Merging the changes from CrowdIn into `shopware/translations` (automated, may require manual review)
3. Merging the changes from `shopware/translations` into `shopware/SwagLanguagePack`, which primarily just distributes json files (automated, may require manual review)
4. Publishing Language Pack releases the store (automated, may require manual input)
...
## Decision
We will implement a new service in `shopware/shopware` (ie as part of the Shopware platform) to download translations right from their [GitHub Repository](https://github.com/shopware/translations/) and manage them without the need to install any extension.
...
In addition, we will provide new bin/console commands in `shopware/shopware` to manage installed languages, for example when building an image for deployment. The initial set of commands will look like:
