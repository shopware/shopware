...
* **Actual cache-control configuration is hard-coded and splattered**: The values set for `cache-control` headers are hard-coded and splattered (e.g., hardly configured in the reverse proxy config and in shopware code), they cannot be configured based on projects needs, and also on route level only the max-age is configurable.
...
The `sw-cache-hash` will be set as a cookie and as a header, additionally we add it as a `vary` header to the response.
Adding it as header allows the default `vary` header implementation of reverse proxies to work.
...
So the configuration (on shopware side, as well on the reverse proxy) will be the same as in the storefront.
...
By removal of the states header those pages will be cached by default. THis might be a breaking change for some projects, when their customizations rely on the cache not being used for logged-in users or filled carts.
This is especially the case when context/user sepecific information is used in the template. In those cases the more dynamic content should be refetched over async AJAX calls to uncached routes, so that the main content can still be cached.
