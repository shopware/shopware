...
- (not an immediate goal of the refactor but still worth considering:) Apps cannot currently provide custom document generators
  or hook into existing ones to modify their output.
...
- [ ] Document types can depend on one another (e.g. Zugferd-PDF can reuse the content of the PDF format +
...
- [ ] Merchant (+Integraitons+Extensions) should still be able to upload document files for a type and
  bypass our generation, like it is now possible (see `document.static` field)
