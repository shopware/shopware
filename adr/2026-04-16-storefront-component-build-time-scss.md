...
* Each bundle's `Resources/views/components/**/*.scss` files are compiled to individual CSS files and placed in `dist-es/components/` alongside their JS counterparts.
* The compiled CSS files are theme-agnostic and are published once to `public/components/` via the `storefront:publish-components` command. Publishing is triggered automatically on plugin and app lifecycle events (install, update, uninstall) so no manual intervention is required during normal operation. The files are not republished on every `theme:compile`.
...
* Extensions (plugins and apps) must ship the pre-compiled CSS files as part of their release artifact. The compiled output from `dist-es/components/` must be included in the extension files so that they can be copied without requiring a build step on the merchant's server.
