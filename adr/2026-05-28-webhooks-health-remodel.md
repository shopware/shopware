...
Per-webhook **four-state health** behind the existing `WEBHOOKS_REWORK` flag. Flag off, the shop is I/O- and behaviour-identical to trunk: the health services are wired into the container either way, and every entry point returns on the flag before its first query. The states:
...
5. **`WEBHOOKS_REWORK` flag** — the Phase 1 flag. Off keeps the legacy failure-handling path. The flag gates behaviour and I/O, not construction: the health service, tick, subscribers and events are registered unconditionally, and each entry point (`WebhookTransport::get()`, the delivery service's result handling, the subscribers, the health routes) returns on the flag before doing any work.
