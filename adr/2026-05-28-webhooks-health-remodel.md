...
Since Phase 1, a business event first becomes a row in a database table (the *outbox*); a worker later picks the row up and `POST`s the payload to the app's URL (synchronous deliveries use the same outbox bookkeeping). This ADR adds a health model on top: it watches each delivery's outcome and decides whether the *next* one may go out. Three parties appear throughout: the **operator** (the merchant, in the Admin), the **app vendor** (who runs the receiving endpoint), and **Shopware** (which delivers).
...
The two planes meet in exactly one place, the `delivery_status` column: health writes it (pause / release / resume / drop), the transport reads it (delivers whatever is claimable). Neither reads the other's columns — this one-column seam is what lets health ship without touching the delivery code.
...
`ErrorClassifier::classify(int $status, ?\Throwable $exception)` returns one enum value — no other inputs, no state, so the same input always gives the same answer. The classifier says what *kind* of failure; *when* to retry is a separate job in `WebhookDeliveryService`, which computes `next_retry_at` from the `RetryDelayCalculator` schedule (`[5 s, 30 s, 5 m, 30 m, 4 h]`, capped at 4 h) and merges a `429`'s `Retry-After` at the failure call site.
...
A breaker must eventually test the endpoint. The canonical way is *half-open*: after a wait, let exactly one request through and judge the result — one at a time, because hammering an endpoint that may still be down just repeats the failure at full volume. Both non-HEALTHY states recover through **one** mechanism:
...
A breaker that trips silently turns hours of shed events into a month-end surprise; pushing a notification on endpoint failure is the industry norm, a poll-only surface the outlier.
...
(Recap for readers landing here first: while your endpoint fails, Shopware *holds* accepted events as paused rows, *sheds* new events during a suspension without recording them, and *cancels* stale held events while keeping their payloads replayable.)
