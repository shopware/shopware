...
| The BC-mirror writes onto `webhook.active`/`error_count` | `Service/WebhookHealthService.php` (the mirror half of `writeRow`) |
| `WebhookActiveFlipSubscriber` (both DAL-write gestures: `active = true` → reactivate, `active = false` → operator kill) + `WebhookHealthService::reactivate()` (Manual trigger) | `Subscriber/WebhookActiveFlipSubscriber.php` — reactivation moves to `POST /api/app-system/webhook/reactivate` and the admin UI, the kill to `POST /api/_action/webhook/{id}/deactivate` |
...
1. **Remove the flag-off branches.** Delete every `Feature::isActive('WEBHOOKS_REWORK')` guard, keeping the
   ...
   `WebhookDeliveryService` / `WebhookEventMessageHandler` / `WebhookActiveFlipSubscriber` /
...
4. **Drop the BC mirror, then the columns.** Delete the mirror half of `writeRow` and the `WebhookEntity` /
   ...
   `WebhookActiveFlipSubscriber`; `POST .../reactivate` is the supported path). *Checkpoint:*
