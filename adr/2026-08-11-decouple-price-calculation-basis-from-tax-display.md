...
- New nullable field `customer_group.price_basis`: `'net'`, `'gross'` or `NULL`.
  ...
  - `'gross'`: the stored gross value stays authoritative. With gross display this is identical to the
    legacy behavior; the missing piece, deriving net prices from a fixed gross for net display, stays
    deferred because `NetPriceCalculator` ignores `isCalculated` and needs its own rounding analysis
    first. The value is only accepted on write when the `v6.8.0.0` flag is active, since it exists for
    the 6.8 end state below.
...
- **Caching**: derived gross prices depend on the country's tax rates, not just the tax state. When the
  customer group uses the `net` basis (the only basis that derives prices), the HTTP cache cookie and the
  entity cache hash additionally include a fingerprint of the context's tax rules; `gross` and legacy
  rows do not fragment the cache.
- **Sorting and filtering** keep using the stored gross column (a per-country gross cannot be indexed).
  ...
  by the rate delta.
- **Admin**: the customer group detail page gets a dedicated "Price display & calculation" card with one
  combined selection (gross prices / net prices / gross prices with net price base) that writes
  `displayGross` and `priceBasis` together, so invalid combinations cannot be configured. A preview in
  the same card visualizes customer price, tax and merchant proceeds for two example tax rates per mode.
  With the `v6.8.0.0` flag the selection writes explicit basis values instead of `null`, already
  producing the 6.8 shape.

End state: `NULL` is transitional. With v6.8 a migration backfills `price_basis` from `display_gross`,
the column becomes `NOT NULL`, and the two fields are fully orthogonal from then on (UPGRADE entry; the
write validation already accepts `'gross'` behind the flag, the gross-to-net derivation for net display
is the remaining prerequisite). Until then `NULL` keeps the old coupling alive for every writer that does
not know the field: old core during blue-green, plugins, ERP syncs, API clients.
...
return match ($basis) {
    // net is authoritative, gross display derives the gross live via GrossPriceCalculator
    CustomerGroupEntity::PRICE_BASIS_NET => new SelectedPrice($price->getNet(), isCalculated: !$displayGross),
    // 'gross', null and unknown values follow the display mode (legacy; gross-to-net derivation deferred)
