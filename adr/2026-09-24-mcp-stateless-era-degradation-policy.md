...
| **Large-result offload (cluster C)** | Honest on both eras via principal-scoped tokens | Re-key off `session_id`. Authorize `resources/read` with **signed short-lived offload tokens** (not raw OAuth `jti`). Ship ResourceLink ([#19966](https://github.com/shopware/shopware/issues/19966)). TTL GC mandatory ([#20511](https://github.com/shopware/shopware/issues/20511)). Cleanup must not assume a session store or DELETE. The Store endpoint also needs its own resource registration and authorization for `resources/read`: today `ToolResultResource` is registered for the Admin server only and the Store registry is tools-only, yet Store tools extend `McpToolResponse` and already offload oversized results. That resource URI cannot be read on `/store-api/_mcp`, so Store needs a Store-scoped resource template (or inline results) as part of this work. |
...
- **Store endpoint:** The offload work adds a Store-scoped tool-result resource template, authorized by the same signed token, so both endpoints follow this policy.
