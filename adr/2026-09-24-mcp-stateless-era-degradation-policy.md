...
The MCP specification changed fundamentally with its 2026-07-28 revision. Clients and SDKs now call the two versions "protocol eras":

**Handshake era** (every revision up to and including 2025-11-25). This is what Shopware serves today.
...
**Modern era** (2026-07-28 and later). The protocol is stateless.
...
The `mcp/sdk` 0.8 used by Shopware serves both eras from the same URL. The client decides which one it speaks, and the server answers in the same era. Shopware currently switches the modern era off for both endpoints (`/api/_mcp` and `/store-api/_mcp`) with `Builder::withoutModernEra()` in `McpServerBuilderCompilerPass`. As soon as that switch is removed, every modern client gets the stateless behaviour described above.
...
| Feature | How it works today | What happens on the modern era |
|---|---|---|
...
**Cleanup and validation.** `DELETE` cleanup and `Mcp-Session-Id` validation stay as they are for handshake clients. Nothing on the modern era may depend on them.

**Multi-step requests.** On the modern era a tool can ask the client for more input and continue in a follow-up request. The SDK keeps the intermediate state in a signed `request_state` value, which needs a secret shared by all workers. That secret must be configured before we offer any feature built on it (for example URL-based confirmation for destructive tools). Those features add to the existing `dryRun` default, they don't replace it.

### Rollout

Both endpoints will serve both eras from the same URL. We don't switch to modern-only, because most clients in the field still speak the handshake era. The modern era is switched on only once the points above are implemented and covered by tests that run the same scenarios against both eras.

## Consequences

**For MCP clients and users.** Handshake clients keep working exactly as today. `?toolsets=` becomes the recommended setup for everyone, because it works on both eras. Modern clients never get a toolset enable that silently does nothing.

**For extension developers.** Tools, prompts and resources don't need to know which era a request uses. The model is the same for the Admin and the Store endpoint. Extensions must not keep their own state keyed by `Mcp-Session-Id`, because it disappears on the modern era. State that must survive between requests has to be keyed by the principal or passed in the request.
...
**For Shopware development.** The result cache is re-keyed from the session to the signed token, the Store endpoint gets a result resource, and `shopware-toolset-enable` is hidden or returns an error on the modern era. Tests have to run the same scenarios against both eras and check the behaviour described here, not whatever the code happens to do. When the implementation lands, `src/Core/Framework/Mcp/AGENTS.md` and `docs/spec-coverage.md` are updated. A future removal of the handshake era is out of scope here. If it happens, it will be announced in advance through the upgrade notes.

Related issues: epic #19965, #20512 (this decision), #19966 (result pointer), #20511 (cleanup by age), #19969 (serving the modern era).
