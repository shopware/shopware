...
Twig parses function calls in inactive template branches before evaluating their feature conditions. Removing a deprecated Twig extension can therefore make current templates fail to compile even when they never call a removed function. `CompatTwigExtension` registers the removed function names only while their major flag is active, and throws if one is called. PHPStan checks literal functions declared by a removed Twig extension against that extension's removal flag in the compatibility map. Templates that still execute a removed function must use the replacement data in their major branch. The deprecated locale filters do not need compatibility registrations because the standard Twig Intl extension remains registered and provides them. None of the removed extensions register Twig tests, operators, or token parsers.
