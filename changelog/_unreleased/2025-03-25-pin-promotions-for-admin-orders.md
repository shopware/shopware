...
* Changed `PromotionCollector` to produce deleted notices for automatic promotion sole for the recalculation process
* Changed `PromotionCollector` to inherit any line-item extension structs from old discount line-items
* Added `RecalculationService::recalculate` to return all cart errors appeared during recalculation
...
___
# Administration
* Changed `sw-order-promotion-field` to have its own card at the general instead of details tab
* Changed the `sw-order-detail` to expose all cart errors in form of notifications
...
No changes will be made to the promotions, other than recalculating the price, unless specifically done manually.
...
The button will reapply _automatic_ promotions:
* Remove _automatic_ promotions whose conditions aren't met anymore
* Add _automatic_ promotions whose conditions apply to the order
___
# Next Major Version Changes
## Toggling of automatic promotions is replaced
