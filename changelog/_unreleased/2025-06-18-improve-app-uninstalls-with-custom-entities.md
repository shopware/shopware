...
---
# Core
* Changed `\Shopware\Core\Framework\App\Lifecycle\AppLifecycle` to explicitly delete custom entities when uninstalling an app, instead of relying on DB level cascade delete constraint. This allows to only update the DB schema if custom entities where removed actually removed, improving the performance for all the uninstalls where no custom entities are affected.
