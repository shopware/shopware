...
* Changed `MailTemplateSetPersister` to write only the parent `mail_template` and `mail_template_type` rows; content is never seeded into the translation table.
...
* Changed `PluginLifecycleService` and `Shopware\Core\Framework\App\Lifecycle\Persister\MailTemplatePersister` to register/remove their declared templates against the registry on install/update/uninstall.
