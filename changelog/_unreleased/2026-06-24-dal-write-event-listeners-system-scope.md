...
* Added `Context::SYSTEM_SCOPE_DAL_WRITE_EVENT` for the temporary DAL write-event scope so private media searches keep normal visibility restrictions. Explicit `$context->scope(Context::SYSTEM_SCOPE, ...)` calls still opt in to private media access.
