...
Use them when a call has many arguments and the values do not describe themselves — `true`, `false`, `0`, `1`, `[]`, `null` — so the reader does not have to open the signature to learn what the third `false` means. Skipping defaults you only passed to reach a later argument is the second reason.
...
Tests in this repository are exempt: naming the arguments of a test data builder or test helper is encouraged, see the *Named arguments for opaque values* entry in [unit-tests.md](./unit-tests.md). A rename breaks those calls in the same commit that renames it, so the BC promise is not what protects them. This does not extend to tests outside this repository, a plugin's for example, which a rename reaches only with a release.
