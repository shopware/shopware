...
Named arguments are useful when calling code with a bad and/or large API. For example, many of PHP's global functions.

The following advantages apply:

* It is possible to skip defaults in between the arguments you want to change.
* The code is better documented since the argument label is specified with the value, very useful for boolean flags. This matters most when a call has many arguments and the values do not describe themselves, `true`, `false`, `0`, `1`, `[]` or `null`, so the reader does not have to open the signature to learn what the third `false` means.
...
Naming an argument couples the call site to the parameter name, so renaming a parameter is a change the callers of supported API can see. Such a rename is announced in advance for a future major version: the current name keeps working until that version, and the announcement records the new name so tooling such as Rector can update the named call sites. Positional call sites are not affected. See [Planning public API changes](feature-flags.md#planning-public-api-changes) for how to announce one.

Named arguments may therefore be used when calling Shopware API's as well as PHP ones.
