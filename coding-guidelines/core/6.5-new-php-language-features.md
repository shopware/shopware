...
> [!NOTE]
> This decision is superseded. Renaming a parameter of supported API is announced ahead of time with the `#[\Shopware\Core\Framework\Deprecation\BCChange\ParameterNameChange]` attribute, which carries the target version and lets Rector migrate named call sites; positional call sites are unaffected. A rename is therefore a managed change rather than a silent break, so the missing promise above no longer rules out naming a Shopware parameter. See the *Deprecations* section of the `shopware-php-code` skill for when to use the BC-change attributes.
