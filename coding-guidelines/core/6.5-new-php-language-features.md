...
Naming an argument couples the call site to the parameter name, so renaming a parameter is a change the callers of supported API can see. Announce such a rename with the `#[\Shopware\Core\Framework\Deprecation\BCChange\ParameterNameChange]` attribute, which records the current name, the future name, and the version the rename happens in, so tooling such as Rector can update the named call sites. Positional call sites are not affected.
