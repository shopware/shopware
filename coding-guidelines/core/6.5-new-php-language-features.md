...
Test code is exempt: naming the arguments of a test data builder or test helper is encouraged, see the *Named arguments for opaque values* entry in [unit-tests.md](./unit-tests.md). A renamed parameter breaks those calls in the same commit that renames it, so the BC promise is not what protects them.
