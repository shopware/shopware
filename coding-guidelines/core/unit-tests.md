...
- **No reflective invocation of private or protected methods** - Do not use reflection (`->invoke()`, `->invokeArgs()`, `setAccessible()`) to call a private or protected method. A non-public method is an implementation detail; test the behavior through the public API, or restructure the code (e.g. extract the logic into a collaborator with a public contract) so it is publicly testable. Reading metadata from a reflection object stays allowed at any visibility, for example asserting a declaring class, a signature, or an attribute. Two checks enforce this: the PHPStan rule `shopware.reflectionOnNonPublicMethod` resolves visibility precisely, and a Danger rule reads only the diff, so it keys on invocation and flags any reflective call in a test, including on a public method — where you should just call the method directly anyway.
