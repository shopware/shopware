...
- **No traits in unit test classes** - A new unit test class does not compose helpers through a trait. Fixture builders go into a static fixture class such as `Shopware\Core\Test\Checkout\CartRuleFixture`, doubles of the class under test into a `Stub/` folder next to the test, and behaviours that boot the kernel stay out of the unit suite. The only trait a unit test class may use is Symfony's `ClockSensitiveTrait`. Environment variables go through `Shopware\Core\Test\TestEnvironment::set()`, restored after every unit test by the FeatureFlag extension; `EnvTestBehaviour` and `EventDispatcherBehaviour` are for the integration suite, where the kernel and the container dispatcher are shared, a unit test owns its dispatcher and calls `addListener()` on it. A Danger rule fails new unit test files composing any other trait; a stub class declared further down in the same file may still use a production trait it exercises.
