 ...
 * @internal
 *
 * EXPERIMENTAL — the Administration extension tooling is shipped for feedback and
 * is not covered by the backwards-compatibility promise. The command names, their
 * options, the generated-file layout and the manifest format can change in any
 * release, so the tooling can be reshaped once real-world usage shows what does
 * not hold. This class is `@internal` on top of that: the bridge mechanism itself
 * (how the ts-node entry point is resolved and spawned) is an implementation
 * detail and must not be extended from outside the platform. No `stableVersion`
 * is declared yet — the marker is dropped here, in the tooling README and in the
 * release note together once the surfaces have settled.
 *
