export default function OrderDeliveryService(client) {
    /**
    ...
    /**
     * @param {String} orderUuid - The order uuid you want the deliveries from
    ...
    function readAll(orderUuid, limit = 25, offset = 0) {
        ...
        const url = `/orderDelivery.${returnFormat}?limit=${limit}&offset=${offset}&query=${queryString}`;
    ...
    function readByUuid(uuid) {
        ...
        return client.get(`/orderDelivery/${uuid}.${returnFormat}`).then((response) => {
            return response.data;
        });
    }
    ...
    function updateByUuid(uuid, payload) {
        ...
        return client.patch(`/orderDelivery/${uuid}.json`, payload).then((response) => {
            return response.data;
        });
    }
