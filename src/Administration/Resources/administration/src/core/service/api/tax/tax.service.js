export default function TaxService(client) {
    /**
    ...
    /**
     * Receives the available taxes from the API end point as a paginated list.
     *
    ...
    function readAll(limit = 25, offset = 0) {
        return client.get(`tax.${returnFormat}?limit=${limit}&offset=${offset}`).then((response) => {
            return response.data;
        });
    }

    /**
     * Receives a tax entry from the API end point.
     *
    ...
    function readByUuid(uuid) {
        ...
        return client.get(`tax/${uuid}.${returnFormat}`).then((response) => {
            return response.data;
        });
    }
