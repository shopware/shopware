...
// The `shopware:*` modules are generated by a Vite plugin, which Jest does not run. One stub per
// specifier stands in for them, written here in the main process so no worker has to create one.
const { writeStubs } = require('./test/_helper_/virtual-shopware-modules/stubs') as {
    writeStubs: (registry: unknown) => number;
};
const { readRegistry } = require('./build/vite-plugins/virtual-shopware-modules/index') as {
    readRegistry: (root: string) => unknown;
};

writeStubs(readRegistry(__dirname));
