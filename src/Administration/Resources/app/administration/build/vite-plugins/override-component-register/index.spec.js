...
describe('build/vite-plugins/override-component-register', () => {
    ...
    it('should transform entry file correctly', () => {
        ...
        // The identifier is positional, so the import line is what identifies the file.
    ...
    it('emits parseable code for paths that differ only in characters an identifier cannot hold', () => {
        // The reported collision: deriving a name from the path had to replace `-`, `_`, `.`, spaces and
        // non-ASCII alike, so `foo-bar/sw-x` and `foo_bar/sw-x` collapsed to the same identifier and the
        // entry declared one binding twice, which does not parse. These fixtures are exactly that case.
        const plugin = OverrideComponentRegisterPlugin({
            root: path.resolve(__dirname, './_fixtures'),
            pluginEntryFile: 'plugin',
        });
        ...
        const result = plugin.transform('// entry', 'plugin');
        const importedPaths = [...result.code.matchAll(/import _swOverride\d+ from '\.\/(.+?)';/g)].map((match) => match[1]);
        ...
        // Duplicate import bindings are a SyntaxError, so parsing is the direct assertion that the
        // generated entry is valid - not just that the identifiers happen to differ.
