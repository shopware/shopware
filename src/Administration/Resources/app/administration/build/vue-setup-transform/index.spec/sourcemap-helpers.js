 ...
 * @returns {number}
 */
function nthIndexOf(source, needle, occurrence = 1) {
    let cursor = -1;
 ...
 *
 * @param {{ code: string, map: { toString(): string } }} result
 * @param {string} generatedNeedle
 * @param {number} [occurrence]
 * @returns {import('source-map-js').NullableMappedPosition}
 */
function originalPositionFor(result, generatedNeedle, occurrence = 1) {
 ...
 * @returns {import('source-map-js').NullableMappedPosition}
 */
function originalPositionForIndex(result, generatedIndex, bias = SourceMapConsumer.GREATEST_LOWER_BOUND) {
    const generatedPosition = positionForIndex(result.code, generatedIndex);
    const consumer = new SourceMapConsumer(JSON.parse(result.map.toString()));
 ...
 *
 * @param {{ code: string, filename: string, map: { toString(): string } }} result
 * @param {string} source
 * @param {string} generatedNeedle
 * @param {string} originalNeedle
 * @param {number} [occurrence]
 * @returns {void}
 */
export function expectOriginalLine(result, source, generatedNeedle, originalNeedle, occurrence = 1) {
 ...
 * @returns {void}
 */
export function expectOriginalPosition(result, source, generatedNeedle, originalNeedle, occurrence = 1) {
    const originalIndex = source.indexOf(originalNeedle);
 ...
 * @returns {void}
 */
export function expectUnmapped(result, generatedNeedle, occurrence = 1) {
    const mappedPosition = originalPositionFor(result, generatedNeedle, occurrence);
 ...
 * @returns {void}
 */
export function expectGeneratedTokenUnmapped(result, generatedNeedle) {
