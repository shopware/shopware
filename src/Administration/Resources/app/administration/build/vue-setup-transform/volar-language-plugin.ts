...
import { inferShopwareSetupFromFilename } from './utils/shopware-setup-block';
 ...
 */
function collectShopwareSetupVolarDiagnostics(fileName: string, sfc: VueSfcDescriptor): ShopwareSetupVolarDiagnostic[] {
    ...
    const { mode } = inferShopwareSetupFromFilename(fileName);
...
function addDiagnosticVirtualCode(code: VolarEmbeddedCode, diagnostics: ShopwareSetupVolarDiagnostic[]): void {
    diagnostics.forEach((diagnostic, index) => {
        ...
        const sourceText = diagnostic.block.content.slice(diagnostic.offset, diagnostic.offset + diagnostic.length);
