 ...
 *   from — the directory of the generated program for runner configs, the
 *   plugin's admin folder for committed configs.
 * - `project`: explicit tsconfig paths for the type-aware rules. Omit it to
 *   let the TypeScript project service locate the nearest tsconfig by walking
 *   up from each linted file — the right mode when the config sits above the
 *   sources. The runner passes its generated program instead, because that
 *   program lives outside the extension tree and no upward walk can find it.
 * - `extensionRoots`: relative paths (from the config file location) that
 *   scope every file glob. Pass the discovered extension sources so the
 *   Twig-Vue processor never touches files outside Administration extensions
 *   (e.g. real server-side Twig templates).
 ...
 *
 * Host options — the Administration's own config can compose this factory too
...
export function shopwareAdminExtension(options = {}) {
    ...
    // Either the project service walks up from each file, or an explicit
    // program is named — never both, they are mutually exclusive.
    const typeAwareParserOptions = project ? { project } : { projectService: true };
