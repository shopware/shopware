...
describe('scripts/codemods/sfc-migration/hoist-block-slots', () => {
    ...
    it('refuses a slot no component owns', () => {
        ...
        expect(result.blockers).toEqual(['named slot inside a twig block cannot be hoisted (no component owns the slot)']);
    });
    ...
    it.each(['sw-modal', 'ChildComponent', 'MtButton'])('hoists a slot owned by %s', (owner) => {
    ...
    it.each(['div', 'span'])('still refuses a slot below the native element %s', (owner) => {
        const result = hoistBlockSlots(`<${owner}><sw-block name="a"><template #footer>X</template></sw-block></${owner}>`);
