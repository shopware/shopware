...
function themeSelect(currentWrapper: VueWrapper): ThemeSelect {
    return currentWrapper.findComponent('.sw-whats-new-modal__theme-select') as unknown as ThemeSelect;
}

describe('src/app/component/structure/sw-whats-new-modal - theme select', () => {
    ...
    it('appears only on the dark mode page', async () => {
        ...
        expect(wrapper.find('.sw-whats-new-modal__theme-select').exists()).toBe(false);

        await wrapper.get('.sw-whats-new-modal__footer-right button').trigger('click');
        await flushPromises();

        expect(wrapper.find('.sw-whats-new-modal__theme-select').exists()).toBe(true);
    });
    ...
    it('notifies the user when the theme cannot be persisted', async () => {
        ...
        expect(createNotificationError).toHaveBeenCalledWith({
            message: 'sw-whats-new-modal.pages.darkMode.themeSaveError',
        });
    });
});
