...
import postInitializer from 'src/app/init-post/index.vite';
...
Object.keys(preInitializer).forEach((key) => {
    ...
    Application.addInitializerVite(key, initializer, '-pre');
});
...
Object.keys(initializers).forEach((key) => {
    ...
    // @ts-expect-error
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    Application.addInitializerVite(key, initializer);
});
...
Object.keys(postInitializer).forEach((key) => {
    ...
    // @ts-expect-error
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    Application.addInitializerVite(key, initializer, '-post');
});
