...
class DocumentApiService extends ApiService {
    ...
    generateDocumentPreviewLink(orderId, orderDeepLink, documentTypeName, config, context) {
        // eslint-disable-next-line max-len
        return `${context.apiPath}/v${context.apiVersion}/_action/order/${orderId}/${orderDeepLink}/document/${documentTypeName}/preview?config=${config}`;
    }

    generateDocumentLink(documentId, documentDeepLink, context, download = false) {
        // eslint-disable-next-line max-len
        return `${context.apiPath}/v${context.apiVersion}/_action/document/${documentId}/${documentDeepLink}${download ? '?download=1' : ''}`;
    }
}
