...
describe('module/sw-customer/page/sw-customer-base-info', () => {
    ...
    it('should display customer information in no edit mode', async () => {
        ...
        // Company
        expect(leftColumn.findAll('sw-description-list-stub').at(0).find('dt').text()).toEqual('sw-customer.baseInfo.labelCompany');
        expect(leftColumn.findAll('sw-description-list-stub').at(0).find('dd').text()).toEqual('Shopware');

        // VAT
        expect(leftColumn.findAll('sw-description-list-stub').at(1).find('dt').text()).toEqual('sw-customer.baseInfo.labelVatId');
        expect(leftColumn.findAll('sw-description-list-stub').at(1).find('dd').text()).toEqual('12345');

        // Customer group
        expect(leftColumn.findAll('sw-description-list-stub').at(2).find('dt').text()).toEqual('sw-customer.baseInfo.labelCustomerGroup');
        expect(leftColumn.findAll('sw-description-list-stub').at(2).find('dd').text()).toEqual('Group test');

        // Default payment method
        expect(leftColumn.findAll('sw-description-list-stub').at(3).find('dt').text()).toEqual('sw-customer.baseInfo.labelDefaultPayment');
        expect(leftColumn.findAll('sw-description-list-stub').at(3).find('dd').text()).toEqual('Payment test');

        // Affiliate code
        expect(leftColumn.findAll('sw-description-list-stub').at(6).find('dt').text()).toEqual('sw-customer.baseInfo.labelAffiliateCode');
        expect(leftColumn.findAll('sw-description-list-stub').at(6).find('dd').text()).toEqual('-');

        // Campaign code
        expect(leftColumn.findAll('sw-description-list-stub').at(7).find('dt').text()).toEqual('sw-customer.baseInfo.labelCampaignCode');
        expect(leftColumn.findAll('sw-description-list-stub').at(7).find('dd').text()).toEqual('-');

        const rightColumn = wrapper.findAll('.sw-customer-base-info-columns').at(1);

        // Customer number
        expect(rightColumn.findAll('sw-description-list-stub').at(0).find('dt').text()).toEqual('sw-customer.baseInfo.labelCustomerNumber');
        expect(rightColumn.findAll('sw-description-list-stub').at(0).find('dd').text()).toEqual('123456789');
    });
});
