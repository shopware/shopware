...
async function createWrapper(repositoryFactory = createMockRepositoryFactory()) {
    return mount(
        await wrapTestComponent('sw-order-list', { sync: true }),
        {
            global: {
                stubs: {
                ...
                mocks: {
                    ...
                    $route: {
                        ...
                        params: {},
                        meta: {
                            $module: { icon: 'regular-shopping-bag' },
                        },
                    },
...
describe('sw-export-channel-tracking extension: sw-order-list', () => {
    let wrapper;
    ...
    describe('listFilters', () => {
        it('calls filterFactory.create with entity "order"', async () => {
            wrapper = await createWrapper();
            await flushPromises();
        ...
        it('uses order-specific label and placeholder snippet keys', async () => {
            wrapper = await createWrapper();
            await flushPromises();
    ...
    describe('getOrderColumns', () => {
        it('appends the Export Channel column', async () => {
            ...
            expect(exportColumn.label).toBe('sw-export-channel-tracking.extension.sw-order-list.list.columnExportFeed');
        });

        it('does not remove any of the original columns', async () => {
            wrapper = await createWrapper();

            const columns = wrapper.vm.getOrderColumns();

            expect(columns.find((c) => c.property === 'orderNumber')).toBeDefined();
        });
    });
