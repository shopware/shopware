...
describe('src/module/sw-order/component/sw-order-new-customer-modal', () => {
    ...
    it('should show error inside sw-tabs-item component', async () => {
        let swTab = wrapper.findAll('.sw-tabs-item').at(0);

        expect(swTab.find('sw-icon-stub').exists()).toBe(false);
        ...
        swTab = wrapper.findAll('.sw-tabs-item').at(0);

        expect(swTab.find('sw-icon-stub[name=solid-exclamation-circle]').exists()).toBe(true);
    });
});
