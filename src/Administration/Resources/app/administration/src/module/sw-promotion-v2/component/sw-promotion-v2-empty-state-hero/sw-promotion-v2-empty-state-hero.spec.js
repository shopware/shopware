...
describe('src/module/sw-promotion-v2/component/sw-promotion-v2-empty-state-hero', () => {
    async function createWrapper(data = {}) {
        return mount(
            await wrapTestComponent('sw-promotion-v2-empty-state-hero', {
                sync: true,
            }),
            {
                props: {
    ...
    it('should render a title', async () => {
        ...
        expect(wrapper.find('.sw-promotion-v2-empty-state-hero__title').text()).toBe('Oh no, nothing was found.');
    });

    it('should render the module description', async () => {
        const wrapper = await createWrapper();

        expect(wrapper.find('.sw-promotion-v2-empty-state-hero__description').text()).toBe(
            'I am some text, which is kinda small, but also somewhat longer than other texts!',
        );
    });
