...
- The tree lists every block currently rendered, nested the way the blocks nest in the page, the way an element inspector shows a document. Each block is tagged as `twig` or `native`, and as `extended` when an override or a native extension already targets it. Filtering matches block and component names and keeps the path to a match. The tree is rebuilt on every route change and follows DOM changes within a page.
...
- The pick action lets you click on any part of the page to select the innermost block underneath it. The click never reaches the page; Escape cancels. The tree then focuses on the picked block, the way an element inspector focuses on a subtree: it shows that block, tagged `picked`, with the blocks nested inside it, and nothing else. The blocks around it are listed in the state panel, and the clear action brings the whole page back. A plugin cannot expand a node in the devtools - a node carries no such flag, and the panel only expands the first root node by itself - so focusing the tree is what puts a picked block in view.
