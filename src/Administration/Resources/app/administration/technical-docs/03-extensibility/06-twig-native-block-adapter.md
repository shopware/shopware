...
```html
<div v-if="$swLegacyBlockIf('demo_block', showCore)">Core case</div>
<div v-if="$swLegacyBlockElse('demo_block')">Legacy fallback</div>
```

The helpers keep a reactive condition chain per host component and block name. This chain stores the result of each case by its absolute case position. A "Twig shim case" is a `v-else-if` or `v-else` case that comes from a legacy Twig block override and is rendered by the generated ShimContent component.

- Native cases pass a stable absolute `caseIndex` generated by the native block transform.
- Twig shim cases also pass an absolute `caseIndex`. The Twig block indexer derives it from the case's local position plus the entry's `caseStartIndex`, so later plugin overrides do not start again at `0`.
- Each Twig shim entry stores `legacyConditionCases` with `caseStartIndex` and `caseCount`. `createShimSlot()` uses this metadata to reserve the pending case range before the shim component renders.

The reconstructed shim template contains generated helper calls such as `$swLegacyBlockElse('demo_block', { caseIndex: 1, isStartingCondition: false, isShim: true })`. Plugin templates do not need any additional syntax; the index metadata is generated while the Twig block is indexed.

Before a shim component renders, `createShimSlot()` reserves the generated case range in the reactive chain. A pending slot is stored as `undefined`, which means: "this case exists, but the shim has not evaluated it yet."
...
For a chain with a Twig shim `v-else-if` followed by a later native fallback, the indexes look like this:

| Step | Source                           | Stored position | Stored value                         |
| ---- | -------------------------------- | --------------- | ------------------------------------ |
| 1    | Native `v-if`                    | `caseIndex: 0`  | `false`                              |
| 2    | Twig shim reservation            | `caseIndex: 1`  | `undefined`                          |
| 3    | Twig shim `v-else-if` evaluates  | `caseIndex: 1`  | `true`                               |
| 4    | Later native fallback            | `caseIndex: 2`  | `false` because position `1` matched |
