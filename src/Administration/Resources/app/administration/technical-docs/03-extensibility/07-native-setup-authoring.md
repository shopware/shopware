...
Override-local bindings are returned under deterministic private aliases only when they are referenced inside `<sw-block extends>` content, forwarded through the reserved `__swOverride` slot-scope channel. The channel is **bound, not destructured**: the slot scope carries the namespace object as `__swSetupOverrideState` and the transform rewrites the content's references to read through it. A slot-scope variable is a `const` in the compiled render function — which is why Vue refuses `v-model` on one — so a destructured binding would make `count++` assign to a copy. Reading through the object puts the write on the reactive state the override returned, where assigning a property that holds a ref sets its value.

A binding **declared** in `swDefineOverride({...})` is a different matter: it becomes the overridden component's state, and the block's data scope hands the content a copy of its value. The transform rejects a template write to one at build time and points at `previousState.<name>.value` in the override setup.
...
- **`<sw-block extends>` content reads override-local state through a generated object.** The transform rewrites each reference to `__swSetupOverrideState.<name>`, so reads unwrap and writes reach the ref. Declared `swDefineOverride({...})` bindings are destructured by name instead, and writing to one is rejected at build time.
