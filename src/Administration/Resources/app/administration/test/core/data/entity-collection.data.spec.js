...
describe('entity-collection.data.js', () => {
    ...
    it('returns first item when first is called', () => {
        const collection = getCollection();

        expect(collection.first()).toStrictEqual(fixture[0]);
    });

    it('return null if collection is empty and first is called', () => {
        const collection = new EntityCollection();

        expect(collection.first()).toBeNull();
    });

    it('return last item when last is called', () => {
        const collection = getCollection();

        expect(collection.last()).toStrictEqual(fixture[fixture.length - 1]);
    });

    it('return null if collection is empty and last is called', () => {
        const collection = new EntityCollection();

        expect(collection.last()).toBeNull();
    });

    it('removes an item depending on the id', () => {
        const collection = getCollection();
    ...
    it('returns false if no item can be removed', () => {
        const collection = getCollection();
    ...
    it('returns true if the collection contains an entity with requested id', () => {
        const collection = getCollection();

        expect(collection.has(fixture[2].id)).toBe(true);
    });

    it('returns false if the collection does not contains an entity with requested id', () => {
        const collection = getCollection();

        expect(collection.has(utils.createId())).toBe(false);
    });

    it('returns the entity with a given id', () => {
        const collection = getCollection();

        expect(collection.get(fixture[2].id)).toStrictEqual(fixture[2]);
    });

    it('returns null if a entity with given id is not found', () => {
        const collection = getCollection();

        expect(collection.get(utils.createId())).toBeNull();
    });

    it('returns the entity at a given index', () => {
        const collection = getCollection();

        expect(collection.getAt(2)).toStrictEqual(fixture[2]);
    });

    it('returns null if no entity is set on a given index', () => {
        const collection = getCollection();

        expect(collection.getAt(-2)).toBeNull();
    });

    it('maps ids properly', () => {
    ...
    it('adds an entity at a given position', () => {
        const collection = getCollection();
    ...
    it('shifts an item if you add at negative position', () => {
        const collection = getCollection();
    ...
    it('does not create holes if you add at index beyond length', () => {
        const collection = getCollection();
    ...
    it('adds item at the end if index is undefined', () => {
        const collection = getCollection();
    ...
    it('pretends moving if oldIndex equals newIndex', () => {
        const collection = getCollection();

        expect(collection.moveItem(2, 2)).toStrictEqual(fixture[2]);
    });

    it('returns null if oldIndex is not valid', () => {
        const collection = getCollection();
    ...
    it('moves an item to the correct position', () => {
        const collection = getCollection();
    ...
    it('shifts an item if new index is smaller 0 without creating holes', () => {
        const collection = getCollection();
    ...
    it('moves an item to the end without creating holes', () => {
        const collection = getCollection();
    ...
    it('does not duplicate collection if new index is undefined', () => {
        const collection = getCollection();
    ...
    it('preserves types after filter', () => {
