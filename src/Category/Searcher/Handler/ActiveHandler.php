...
class ActiveHandler implements HandlerInterface, AggregatorInterface
{
    ...
    public function handle(
        ...
        if ($criteriaPart instanceof ActiveSorting) {
            $builder->addOrderBy('category.active', $criteriaPart->getDirection());

            return;
        }

        /* @var ActiveCondition $criteriaPart */
        $builder->andWhere('category.active = :active_condition');
    ...
    public function aggregate(
        ...
        $builder->groupBy('category.active');

        $builder->select(['category.active', 'COUNT(category.uuid) as item_count']);
