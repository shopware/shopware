...
class CustomerDefinition extends EntityDefinition
{
    ...
    protected function defineFields(): FieldCollection
    {
        return new FieldCollection([
            ...
            (new ManyToOneAssociationField('defaultBillingAddress', 'default_billing_address_id', CustomerAddressDefinition::class, 'id', false))->addFlags((new ApiAware())->withDescription('Default billing address for the customer'), new SearchRanking(SearchRanking::ASSOCIATION_SEARCH_RANKING)),
            (new ManyToOneAssociationField('activeBillingAddress', 'active_billing_address_id', CustomerAddressDefinition::class, 'id', false))->addFlags((new ApiAware())->withDescription('Currently active billing address in the session'), new Runtime()),
            (new ManyToOneAssociationField('defaultShippingAddress', 'default_shipping_address_id', CustomerAddressDefinition::class, 'id', false))->addFlags((new ApiAware())->withDescription('Default shipping address for the customer'), new SearchRanking(SearchRanking::ASSOCIATION_SEARCH_RANKING)),
            (new ManyToOneAssociationField('activeShippingAddress', 'active_shipping_address_id', CustomerAddressDefinition::class, 'id', false))->addFlags((new ApiAware())->withDescription('Currently active shipping address in the session'), new Runtime()),
            (new ManyToOneAssociationField('salutation', 'salutation_id', SalutationDefinition::class, 'id', false))->addFlags((new ApiAware())->withDescription('Customer salutation (e.g., Mr., Mrs., Ms.)')),
            (new OneToManyAssociationField('addresses', CustomerAddressDefinition::class, 'customer_id', 'id'))->addFlags((new ApiAware())->withDescription('All addresses saved for the customer'), new CascadeDelete()),
            (new OneToManyAssociationField('orderCustomers', OrderCustomerDefinition::class, 'customer_id', 'id'))->addFlags(new SetNullOnDelete()),
            (new ManyToManyAssociationField('tags', TagDefinition::class, CustomerTagDefinition::class, 'customer_id', 'tag_id'))->addFlags(new SearchRanking(SearchRanking::ASSOCIATION_SEARCH_RANKING), (new ApiAware())->withDescription('Tags assigned to the customer for organization and segmentation')),
