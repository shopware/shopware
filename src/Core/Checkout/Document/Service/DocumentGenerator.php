...
class DocumentGenerator
{
    ...
    public function readDocument(
        ...
        if ($resolvedFile !== null) {
            $fileBlob = $context->scope(Context::SYSTEM_SCOPE, fn (Context $context): string => $this->mediaService->loadFile($resolvedFile->media->getId(), $context));

            $renderedDocument = new RenderedDocument(
                name: $resolvedFile->fileName . '.' . $resolvedFile->fileExtension,
        ...
        // V2 documents must not be regenerated by the V1 renderer as their config may not contain all V1 rendering data.
        if (($document->getDocumentFiles()?->count() ?? 0) > 0) {
            return null;
        }
