...
/**
 * Assembles the shared invoice-like render data from an order.
 *
 * @internal
 */
#[Package('after-sales')]
final readonly class InvoiceRenderDataFactory
{
    ...
    public function build(
        ...
        $documentType = DocumentType::tryFrom($generationRequest->documentType);
        $allowNegative = $documentType?->allowsNegativeLineItems() ?? false;
        $allowCredit = $documentType?->allowsCreditLineItems() ?? false;

        $lineItems = LineItemView::listFromOrder($order, $allowNegative, $allowCredit);
        $allowanceCharges = $allowCredit ? [] : AllowanceChargeView::listFromOrder($order);
    ...
    private function isIntraCommunityDelivery(bool $displayAdditionalNoteDelivery, OrderEntity $order): bool
    {
        ...
        if (!$country->getIsEu() || !$country->getCompanyTax()->getEnabled()) {
            return false;
        }
