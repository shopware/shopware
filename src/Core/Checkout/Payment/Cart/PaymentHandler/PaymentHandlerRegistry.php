...
class PaymentHandlerRegistry
{
    ...
    public function __construct(
        ...
        foreach (\array_keys($syncHandlers->getProvidedServices()) as $serviceId) {
            trigger_deprecation('shopware/core', '', 'The tag `shopware.payment.method.sync` is deprecated and will be removed in 6.7.0. Use `shopware.payment.handler` instead.');
            $handler = $syncHandlers->get($serviceId);
            $this->handlers[(string) $serviceId] = $handler;
        }

        foreach (\array_keys($asyncHandlers->getProvidedServices()) as $serviceId) {
            trigger_deprecation('shopware/core', '', 'The tag `shopware.payment.method.async` is deprecated and will be removed in 6.7.0. Use `shopware.payment.handler` instead.');
            $handler = $asyncHandlers->get($serviceId);
            $this->handlers[(string) $serviceId] = $handler;
        }

        foreach (\array_keys($preparedHandlers->getProvidedServices()) as $serviceId) {
            trigger_deprecation('shopware/core', '', 'The tag `shopware.payment.method.prepared` is deprecated and will be removed in 6.7.0. Use `shopware.payment.handler` instead.');
            $handler = $preparedHandlers->get($serviceId);
            $this->handlers[(string) $serviceId] = $handler;
        }

        foreach (\array_keys($refundHandlers->getProvidedServices()) as $serviceId) {
            trigger_deprecation('shopware/core', '', 'The tag `shopware.payment.method.refund` is deprecated and will be removed in 6.7.0. Use `shopware.payment.handler` instead.');
            $handler = $refundHandlers->get($serviceId);
            $this->handlers[(string) $serviceId] = $handler;
        }

        foreach (\array_keys($recurringHandlers->getProvidedServices()) as $serviceId) {
            trigger_deprecation('shopware/core', '', 'The tag `shopware.payment.method.recurring` is deprecated and will be removed in 6.7.0. Use `shopware.payment.handler` instead.');
