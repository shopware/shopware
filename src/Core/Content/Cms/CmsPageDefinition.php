...
class CmsPageDefinition extends EntityDefinition
{
    ...
    protected function defineFields(): FieldCollection
    {
        return new FieldCollection([
            ...
            (new OneToManyAssociationField('sections', CmsSectionDefinition::class, 'cms_page_id'))->addFlags((new ApiAware())->withDescription('Content sections within the CMS page (layout blocks containing slots)'), new CascadeDelete()),
            (new TranslationsAssociationField(CmsPageTranslationDefinition::class, 'cms_page_id'))->addFlags(new ApiAware()),
            (new ManyToOneAssociationField('previewMedia', 'preview_media_id', MediaDefinition::class, 'id', false))->addFlags((new ApiAware())->withDescription('Preview image for the CMS page in admin panel and page selection')),

            (new OneToManyAssociationField('categories', CategoryDefinition::class, 'cms_page_id'))->addFlags(new RestrictDelete()),
            (new OneToManyAssociationField('landingPages', LandingPageDefinition::class, 'cms_page_id'))->addFlags((new ApiAware())->withDescription('Landing pages using this CMS layout'), new RestrictDelete()),
