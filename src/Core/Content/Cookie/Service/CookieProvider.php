...
class CookieProvider
{
    ...
    final public const COOKIE_ENTRY_CONSENT_ID_COOKIE = 'cookie-consent-id';
    ...
    public function getCookieGroups(Request $request, SalesChannelContext $salesChannelContext): CookieGroupCollection
    {
        ...
        $this->scriptExecutor->execute(new CookieGroupCollectHook($cookieGroups, $salesChannelContext));
    ...
    /**
     * Holds the token that links a visitor's decisions in the server-side consent log.
     * Listed visibly, because a visitor needs it to retrieve their own records, and kept
     * as long as the records themselves. The value is generated by the client, so the
     * entry carries none.
     */
    private function getRequiredConsentIdEntry(): CookieEntry
    {
        $entryRequiredConsentId = new CookieEntry(self::COOKIE_ENTRY_CONSENT_ID_COOKIE);
        $entryRequiredConsentId->name = 'cookie.groupRequiredConsentId';
        $entryRequiredConsentId->expiration = $this->consentLogRetentionDays;

        return $entryRequiredConsentId;
    }
