...
StoreAPI component schemas are being migrated from PHP-generated base schemas to JSON files. JSON should become the source of truth for core StoreAPI schemas. The PHP side must remain available for extension metadata, especially `properties.extensions`, until the generator can extract extension information without generating full base schemas.
...
- `jsonOverridesPhpGenerated`: a JSON schema exists for a schema name that PHP still generates. The final StoreAPI schema uses the JSON base schema and keeps PHP-generated extension metadata.
...
- `jsonWithoutPhpGenerated`: JSON-only schemas. These are already independent from PHP base generation.
- `*WithoutAllowlist`: mismatches that are not accepted by the current allowlist and should fail CI when `--fail-on-mismatch` is used.
- `allowlistWithout*`: stale allowlist entries. Remove these entries when the corresponding migration work makes them obsolete.

The core allowlist lives in `Generator/StoreApiPhpGeneratedSchemaAllowlist.json`. It is a temporary migration aid, not the target state. When a schema is migrated, update the JSON schema files and remove the matching allowlist entry in the same change. The target for core is that `jsonOverridesPhpGeneratedWithoutAllowlist`, `phpGeneratedOnlyWithoutAllowlist`, and the stale allowlist buckets stay empty in CI, and the allowlist itself shrinks over time.
