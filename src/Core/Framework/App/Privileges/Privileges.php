...
class Privileges
{
    ...
    public function requestPrivileges(string $appId, array $privileges, Context $context): void
    {
        ...
        $existingPrivileges = json_decode($existingPrivileges, true, 512, \JSON_THROW_ON_ERROR);
    ...
    private function decodePrivileges(array $privileges): array
    {
        return array_map(
            static fn (?string $appPrivileges) => $appPrivileges
                ? json_decode($appPrivileges, true, 512, \JSON_THROW_ON_ERROR)
                : [],
            $privileges
        );
    }
    ...
    private function fetchPrivileges(array $appIds): array
    {
        ...
        return array_map(static fn (array $row): array => [
            json_decode($row['privileges'], true, 512, \JSON_THROW_ON_ERROR),
            json_decode($row['requested_privileges'], true, 512, \JSON_THROW_ON_ERROR),
        ], $privileges);
    }
