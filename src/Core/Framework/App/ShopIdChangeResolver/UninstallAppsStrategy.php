...
class UninstallAppsStrategy extends AbstractShopIdChangeStrategy
{
    ...
    public function resolve(Context $context): array
    {
        ...
        foreach ($this->appRepository->search(new Criteria(), $context)->getEntities() as $app) {
            // Snapshot id+name before delete so downstream listeners (via ShopIdResolvedEvent) still have the
            // technical name they need (e.g. for theme cleanup) even though the row is gone.
