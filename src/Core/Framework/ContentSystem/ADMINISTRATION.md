...
> **TODO (versioning):** `expectedVersion` is a pragmatic interim token built on the row's `updatedAt`, compared at millisecond precision (the storage precision). It catches a single stale-read client, but it is not a true compare-and-swap: two writers that both read the same revision (at any timing, not just within one millisecond) both pass the `updatedAt` check, then each commits an `update()` keyed by primary key alone, so the second write overwrites the first and that first edit is silently lost. A real layout versioning system (draft/published revisions with explicit version identifiers) is planned and will supersede this token and close that lost-update window.
>
