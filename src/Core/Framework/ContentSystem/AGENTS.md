...
- **Resolvability gate** (DAL `PreWriteValidationEvent`): `Validation/ContentLayoutWriteValidator` (well-formedness on `content_layout` + synchronous re-check of bound layouts via tagged `Binding/LayoutBindingEnumerator`s) and `Validation/ContentLayoutAssignmentWriteValidator` (binding gate on entity assignments, delegating to the shared `Validation/LayoutBindingGate`). Layout decoding runs through the shared `Validation/LayoutTreeDecoder`, injected directly into `Validation/ContentLayoutWriteValidator` and `Validation/LayoutBindingGate` (the assignment validator does not decode itself — it delegates to `LayoutBindingGate`). `Validation/LayoutResolvabilityValidator` exposes the two gate entry points `wellFormedness()` / `resolvability()`, each returning a `Diagnostics/DiagnosticsReport` whose `isWellFormed()` / `isResolvable()` are the actual predicates; its `isBindingEnforced(BoundRootContext)` seam (default enforces every binding) is the single override point for a future draft/versioning system. `LayoutResolvabilityValidator::SKIP_VALIDATION_STATE` write-context flag bypasses both gates
...
- **Resolve-and-diagnose route**: `Api/ContentLayoutDiagnosticsController` (`POST /api/_action/content-system/layout/diagnose`); source selection via `Api/SpecificationSourceResolver` (`entityType` or `section`)
