...
- `ContentLayoutDiagnosticsController` - Route `POST /api/_action/content-system/layout/diagnose` (name `api.action.content_system.layout.diagnose`), scope `ApiRouteScope`; returns `{ resolutions, diagnostics }` without persisting. Optional `entityType`/`section` enable the binding-resolvability branch via `SpecificationSourceResolver`
- `ContentPreviewRequest` / `ContentLayoutDiagnoseRequest` - envelope DTOs; each controller action binds its DTO via `#[MapRequestPayload(validationFailedStatusCode: Response::HTTP_BAD_REQUEST)]` on the **action parameter** (the attribute is on the parameter, not on the DTO class)
