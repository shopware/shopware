...
`DiagnoseResponse` is the `\JsonSerializable` sibling of `MutationResponse`, owning this route's two-key shape. `fromReport(array $resolutions, DiagnosticsReport $report)` normalizes the raw resolutions plus the already-merged report; `jsonSerialize()` casts `resolutions` to `{}`-when-empty. Same output-only discipline as `MutationResponse`.

`LayoutDiagnosticsResultNormalizer` is the zero-dependency (`new`-able) normalizer behind both: `normalizeResolutions()` for the resolutions map, `normalizeReport()` for the diagnostics report. It is instantiated inside the `MutationResponse::fromResult()` and `DiagnoseResponse::fromReport()` factories rather than by a controller, so the diagnostics half of every admin content response shares one wire shape.
