...
A client derives the specifications applicable to an element from the `bindingSpecifications` map on that element's type entry in [`content-system-element-types.json`](../../ADMINISTRATION.md#element-types) (`bindingSpecifications[element.component]`) — the ids from the [Binding specifications](../../ADMINISTRATION.md#binding-specifications) fold that a client may pass as `bindingSpecificationId` to a bind-element action.
