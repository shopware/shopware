...
`invalid_mapping` flags a stored data mapping the bound source's candidate catalogue does not admit, with `key` carrying the mapped property. Four conditions produce it: the property does not declare `mappable`, the catalogue does not offer the mapped path, the mapping's `projection` is not the one the catalogue pairs with that path, or the candidate's value type cannot fill the declared property type. It is binding-scope because a catalogue belongs to a root source — the same mapping can be admissible under one bound source and not another — so it appears only when the request names a `rootSource`, and a request without one leaves mappings unjudged rather than reporting them as broken.

The same four conditions refuse the `content_layout` write, where they surface as `CONTENT_SYSTEM__PROPERTY_NOT_MAPPABLE`, `CONTENT_SYSTEM__UNKNOWN_MAPPING_PATH`, `CONTENT_SYSTEM__MAPPING_PROJECTION_MISMATCH`, or `CONTENT_SYSTEM__MAPPING_TYPE_MISMATCH` on the property path. The two surfaces read one rule set (`Mapping/StoredMappingInspector`) and cannot disagree about which mappings are legal; they differ only in how much they say, because a write needs a code a client can branch on while an editor needs to mark a control.
