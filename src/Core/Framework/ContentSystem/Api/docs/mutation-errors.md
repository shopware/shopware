...
| An element-type, style-option, or binding registry used by the operation cannot validate its loaded server-side definitions | 500 | The corresponding `ELEMENT_TYPE_LOAD_FAILED`, `STYLE_OPTION_LOAD_FAILED`, or `BINDING_SPECIFICATION_LOAD_FAILED` error propagates |
