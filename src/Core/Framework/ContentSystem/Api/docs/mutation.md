...
| `wrap-elements`     | `WrapElementsRequest` | `elementIds` (required, a non-empty list of ids that are siblings in one slot, or all roots; validated `#[Assert\All([new Assert\Type('string'), new Assert\NotBlank()])]` + `#[Assert\Unique]`); `containerType` (required); `slot` (required)                                                         |
