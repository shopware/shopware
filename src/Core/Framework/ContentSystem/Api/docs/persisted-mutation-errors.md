...
| The server-side definitions for an element-type, style-option, or binding registry fail validation or otherwise cannot be loaded | 500 | The corresponding `ELEMENT_TYPE_LOAD_FAILED`, `STYLE_OPTION_LOAD_FAILED`, or `BINDING_SPECIFICATION_LOAD_FAILED` error propagates instead of being relabelled as an invalid layout write |
