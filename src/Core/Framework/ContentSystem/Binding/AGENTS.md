...
- `LayoutBindingEnumerator::enumerate()` PHP return type is `array`; docblock contract is `list<SourceBinding>` — implementations must return a list (no string keys)
- `SourceBinding::$sourceId` is the source identifier — the bound entity type for entity bindings (e.g. `product`, `category`) or the section name for storefront bindings (`header`, `footer`); `$providedRootContext` is the ambient context the source supplies to the layout's top-level elements
...
- Related: `Validation/LayoutGate::isBindingEnforced(SourceBinding)` is the consumer of each `SourceBinding`; `Resolution/ProvidedContext` is the element type of `$providedRootContext`
