...
- `LayoutBindingEnumerator` — interface tagged `content_system.layout_binding_enumerator`; implement and tag to add a new binding source; `enumerate(string $contentLayoutId, Context $context): array` returns `list<BoundRootContext>` — return an empty list when the layout has no bindings, including a not-yet-persisted layout id on the CREATE path, where `Validation/ContentLayoutWriteValidator` passes a brand-new id with zero existing assignments
- `BoundRootContext` — `@internal` value object carrying `$sourceId` (a source identifier — the bound entity type for entity bindings, e.g. `product`/`category`, or the section name `header`/`footer`) and `$providedRootContext` (`list<ProvidedContext>`); consumed by `Validation/LayoutResolvabilityValidator`
