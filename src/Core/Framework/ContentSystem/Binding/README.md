...
The invariant is **at most one promoted specification per element type**, enforced at three points:
...
- **Hard, at app install.** `App/Lifecycle/Persister/ContentSystemBindingSpecificationPersister` runs the same own-set check on the dtos it persists, thrown directly as the install's `AppException`. This is not redundant with the validator below: the loader's own check lives in `load()`, which the persister's raw-dto path bypasses, and the programmatic/store install path never runs manifest validation — without this check two promoted rows would persist and reach the prod catalog unreconciled.
- **Soft, in the app validator.** `App/Validation/ContentSystemBindingSpecificationAppValidator` reports a schema error (never an exception) when an app promotes one type twice across its own inline specifications — an own-set check, independent of the aggregated registry, so `manifest:validate` reports what install would reject.

An inline specification's implicit type is always its containing file's source-prefixed type, so two different sources can never promote the same type; the app-install check closes the only gap the loader's check cannot see (an app's own set, persisted without `load()` ever running).
