...
- `LayoutDiagnostics` - Entry point. `analyze(array $tree, ?array $rootContext, ?string $rootSource = null): LayoutAnalysis`. Pass `null` for `$rootContext` to run the intrinsic subset only (well-formedness check without a bound source). `$rootSource` is separate and optional because the data-mapping checks are keyed on the source ID rather than on the resolved context: a caller that has the ID gets them, one that does not leaves mappings unjudged.
...
`InvalidMapping` flags a stored data mapping the bound source's candidate catalogue does not admit: a property that does not declare `mappable`, a path the catalogue does not offer, a `projection` that is not the one the catalogue pairs with that path, or a candidate value type the property cannot hold. Keyed on the mapped property, so the entry names the control the author acted on rather than a wiring key the editor does not render.

The rules live in `Mapping/StoredMappingInspector` and not here, because two surfaces apply them: this one reports, and `Validation/StoredMappingValidator` refuses the write with a specific `CONTENT_SYSTEM__*` code per rule. That split is why the code exists at all — before it, a mapping the gate could already prove wrong was silent in the Experience Studio and only surfaced on save, which is what the gallery-versus-single-media confusion looked like from the author's side.
