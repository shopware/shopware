...
- Preparation order a listener is positioned against: language reduction first, then placeholder resolution, both in FULL mode only, both after `ContentTreePreparationEvent`. A preparation listener therefore reads and writes a translatable property as its whole language map, keyed by language id; it MUST NOT write a bare string there, and a value the listener leaves non-map reaches reduction as `TRANSLATION_SHAPE_INVALID` (500)
