...
- `key` (optional) - Names this data requirement. It does NOT place the loaded value: the element property the data lands under is the object's key name in the `dataRequirements` map, always. `Layout/Codec/StoredElementCodec` keys the decoded map by the map key (`StoredElementCodec.php:259`) and `Rendering/RenderedElementFactory` mints from those map keys (`RenderedElementFactory.php:105`), so an inner `key` differing from its map key changes nothing about placement. Omitted, it falls back to the map key.
