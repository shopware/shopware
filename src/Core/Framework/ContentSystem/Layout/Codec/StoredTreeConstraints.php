...
final class StoredTreeConstraints
{
    ...
    private function elementConstraints(int $depth): array
    {
        return [
            ...
            new Callback($this->validateRedistributeProviderConflicts(...)),
        ];
    }
     ...
     * overwrite the other. {@see StoredElementCodec} throws on the first collision; here every colliding
     ...
     *
     * The rule spans both wiring maps of one element, which is why it is attached whole-element in
     * {@see elementConstraints()} rather than beside the consumer map's own rules.
     */
    private function validateRedistributeProviderConflicts(mixed $value, ExecutionContextInterface $context): void
    {
        if (!\is_array($value)) {
            return;
        }
