 ...
 *
 * It exists to hold the one place where a storage-typed tree still meets code written against the older model:
 * `ContentSystem\ContentPipeline::load()`, which lowers once the stored preparation steps have run and speaks
 * the older model from there on. Both layouts reaching it — the served one and the previewed draft — arrive
 * as stored trees, so the seam is a single line inside the pipeline rather than one per producer.
 *
 * That call goes when {@see ContentElement} is retired entirely, and this class is deleted with it. The site
 * above is the whole set: an undeclared call site would make the old model reachable from somewhere the split
 * has already moved past, so a new one is added here with its expiry or not added at all. Validation, mutation
 * and diagnostics lower nothing: they take and return stored elements.
 *
 * @internal
 */
#[Package('framework')]
