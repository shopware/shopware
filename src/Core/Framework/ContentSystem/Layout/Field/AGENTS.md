...
- `ContentElementListFieldSerializer` overrides `normalize` to seed the element types' primitive defaults into the layout payload via the injected `Layout/LayoutDefaultMaterializer` (the `IdFieldSerializer::normalize` precedent). It runs before `PreWriteValidationEvent`, handles both `ContentElement` and raw-array payloads, and never overwrites an authored value
