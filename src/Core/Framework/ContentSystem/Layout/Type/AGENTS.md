...
- `mappable` defaults to **false** and has no validator: it is a pure opt-in, and that default is what keeps a property the element fills itself (via `dataRequirements`, a binding specification, or page-supplied root-ambient context, as with `Sw:Product:Listing`'s `listing`) out of the author's reach. `Validation/StoredMappingValidator` refuses any write mapping a property that did not opt in. Marking a property both mappable and self-filled is legal but silently lets a mapping win, since `Output/Index/ValueOrigin` ranks `DeliveredContext` above `LoaderResolved` — see [docs/custom-types.md](docs/custom-types.md#mappable)
