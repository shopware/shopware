...
- `DatabaseTypeLoader` joins `app` and queries `WHERE app.active = 1` — a deactivated app's types drop from that query with no extra write (no denormalized `active` column on the type), though the cached registry keeps serving them until its next invalidation (the persister on a later app install/update). `ElementTypeCollisionDetector` also considers types of inactive apps to prevent name collisions across apps. Collision check is best-effort (TOCTOU window); the `UNIQUE KEY` on `app_content_system_element_type.name` is the authoritative guard. A persisted row whose schema fails to decode or validate is skipped and logged at `warning` level rather than failing the whole load — unlike `YamlTypeLoader`, which fails hard on an authored file.
