...
5. **Finishing** — `ContentPipeline` finishes the rendered tree itself: virtual root cleanup, partial extraction, both driven by the `RenderScaffolding` recorded during preparation. `Layout/Element/ContentElementLowering` then bridges the finished rendered forest, together with the stored forest it was minted from, back onto `ContentElement` — the model the response and `PostHydrationEvent` still speak. The bridge pairs the two forests by element id, because the finishing steps remove nodes and so leave position and root count no longer comparable. `PostHydrationEvent` is dispatched after the bridge, so a listener sees the finished tree. See Event/Listener/.
