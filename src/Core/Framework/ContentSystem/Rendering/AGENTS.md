...
- `ElementDataResolver` - runs ONE element's data requirements; `resolve(StoredElement $stored, SalesChannelContext $context, Request $request, RenderingCacheContext $cacheContext, array $deliveredContext = []): array<string, ResolvedLoaderValue>` keyed by requirement key. Mints each value's `Output/Index/LoaderValueIdentity` and applies each loader result's cacheability to the cache context
- `ContextDeliveryResolver` - resolves the context one loaded parent delivers to its direct children during lowering via `resolveDirectChildren()`. Its whole-forest `resolve()` entry point remains available for callers that already hold every loader value
...
- In FULL mode, each parent resolves its data before delivering context to its direct children, and each child resolves its data only after receiving that context. `ElementLowering::lower()` owns this depth-first order so a child loader can consume a value its parent provided
- `ElementDataResolver` resolves each requirement's `LoaderInputs` (via `LoaderInputResolver`, from the loader's `configSpecification()`, the requirement's config, the element's unwrapped `StoredElement::properties()`, and its delivered context) before calling `load()`
