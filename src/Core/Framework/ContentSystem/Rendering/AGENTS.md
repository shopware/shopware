...
- `ContextDeliveryResolver` - owns the forest walk (top-down, required for chained re-providing); `resolve(list<StoredElement> $forest, array<string, array<string, mixed>> $loaderValues, array<string, mixed> $ambientContext): ContextDeliveryIndex`, and resolves direct-child deliveries during lowering
...
- In FULL mode, each parent resolves its data before delivering context to direct children, and each child resolves its data only after receiving that context (`ElementLowering::lower()` owns this order)
- `ElementDataResolver` resolves each requirement's `LoaderInputs` from the loader config, the element's unwrapped stored properties, and delivered context before calling `load()`
