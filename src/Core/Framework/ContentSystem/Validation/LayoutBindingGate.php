...
/**
 * Shared binding-gate core: resolves a content layout's tree by id and returns the binding-scope violations it
 * has against a source's provided root context. The Core entity-assignment gate and the Storefront header/footer
 * gate both delegate here, so the "resolve tree → resolvability → map" path is single-sourced across sections.
 *
...
class LayoutBindingGate
{
     ...
     * deliberately swallowed: the well-formedness gate ({@see ContentLayoutWriteValidator}) already records it for
     * the same in-batch command, so re-reporting here would duplicate the violation.
     *
