...
- **`*Lowering`** is a one-way translation from a richer model to a poorer one, a noun because the translation itself is the thing: `ContentElementLowering` takes a stored element together with the rendered element minted from it down onto the older `ContentElement` model, and `ElementLowering` takes a stored forest down onto the rendered one. Information is dropped by design and no inverse exists; a role with an inverse is a `Codec`.
