...
class VersionManager
{
    ...
    private function cloneEntity(
        ...
        if ($createdAtField instanceof DateTimeField) {
            $data['createdAt'] = \DateTime::createFromImmutable($this->clock->now());
        }

        if ($updatedAtField instanceof DateTimeField) {
            if ($updatedAtField->getFlag(Required::class)) {
                $data['updatedAt'] = \DateTime::createFromImmutable($this->clock->now());
            } else {
                $data['updatedAt'] = null;
            }
        }
    ...
    private function writeAuditLog(array $writtenEvents, WriteContext $writeContext, ?string $versionId = null, bool $isClone = false): void
    {
        ...
        $date = \DateTime::createFromImmutable($this->clock->now())->format(Defaults::STORAGE_DATE_TIME_FORMAT);
    ...
    private function updateVersionData(VersionCommitCollection $commits, WriteContext $writeContext, string $versionId): void
    {
        ...
        foreach ($commits as $commit) {
            foreach ($commit->getData() as $data) {
                ...
                $new[] = [
                    ...
                    'createdAt' => \DateTime::createFromImmutable($this->clock->now())->format(Defaults::STORAGE_DATE_TIME_FORMAT),
                ];
            }
        }
        ...
        $commit = [
            ...
            'isMerge' => true,
            'message' => 'merge commit ' . \DateTime::createFromImmutable($this->clock->now())->format(Defaults::STORAGE_DATE_TIME_FORMAT),
        ];
