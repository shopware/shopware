...
Handshake-era lifecycle methods `initialize` and `notifications/initialized` are exempt from the limiter (narrow allowlist; mixed JSON-RPC batches still count). `time_backoff` accepts only one request after a wait, so without the exemption a standards-compliant Streamable HTTP handshake cannot complete once the first threshold is crossed (#18906). Tool calls and other methods stay limited. The modern (2026-07-28) era has no such pair.
...
- **`time_backoff` vs token bucket** — Shopware uses the `time_backoff` policy (escalating penalty on repeated hits), not a token bucket, so it does not give the "burst then sustained" shape the MCP guides recommend for bursty agent traffic. Handshake methods are exempt (above); revisit the policy if agents still hit the limit on legitimate tool fan-out.
