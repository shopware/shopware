...
Only the handshake-era follow-up `notifications/initialized` is exempt from the limiter (single message or a batch containing only that notification; `initialize` and mixed batches still count). `time_backoff` accepts only one request after a wait, so once `initialize` has consumed that slot the mandatory follow-up would otherwise get HTTP 429 (#18906). `initialize` stays rate-limited to prevent endless session creation. Tool calls and other methods stay limited. The modern (2026-07-28) era has no such pair.
