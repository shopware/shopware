...
The handshake-era follow-up `notifications/initialized` (single message or a batch containing only that notification) draws from its own `mcp_initialized_notification` bucket instead: a small `sliding_window` keyed per OAuth token (Admin) or per IP (Store). `time_backoff` accepts only one request after a wait, so once `initialize` has consumed that slot the mandatory follow-up would otherwise get HTTP 429 (#18906). The separate bucket keeps the follow-up bounded, so it is not an unlimited path through the endpoint. `initialize`, mixed batches, tool calls and all other methods stay on the endpoint bucket. The modern (2026-07-28) era has no such pair.
