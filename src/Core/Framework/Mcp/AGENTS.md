...
The handshake-era follow-up `notifications/initialized` (single message or a batch containing only that notification) draws from its own key on the same route (`initialized-` plus the OAuth token on Admin, plus the client IP on Store), so it gets the endpoint limits without sharing the `initialize` counter and without extra configuration. `time_backoff` accepts only one request after a wait, so once `initialize` has consumed that slot the mandatory follow-up would otherwise get HTTP 429 (#18906). The separate key keeps the follow-up bounded, so it is not an unlimited path through the endpoint. `initialize`, mixed batches, tool calls and all other methods stay on the endpoint key. The modern (2026-07-28) era has no such pair.
...
- **`time_backoff` vs token bucket** — Shopware uses the `time_backoff` policy (escalating penalty on repeated hits), not a token bucket, so it does not give the "burst then sustained" shape the MCP guides recommend for bursty agent traffic. Only `notifications/initialized` has its own key (above); revisit the policy if agents still hit the limit on legitimate tool fan-out.
