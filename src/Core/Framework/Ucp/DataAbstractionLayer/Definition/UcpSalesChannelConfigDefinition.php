...
class UcpSalesChannelConfigDefinition extends EntityDefinition
{
    ...
    protected function defineFields(): FieldCollection
    {
        return new FieldCollection([
            ...
            (new StringField('ucp_version', 'ucpVersion'))->addFlags(new ApiAware(), new Required()),
            (new StringField('profile_uri_strategy', 'profileUriStrategy'))->addFlags(new ApiAware(), new Required()),
            (new StringField('custom_profile_uri', 'customProfileUri'))->addFlags(new ApiAware()),
            (new JsonField('enabled_capabilities', 'enabledCapabilities'))->addFlags(new ApiAware(), new Required()),
            (new JsonField('enabled_transports', 'enabledTransports'))->addFlags(new ApiAware(), new Required()),
            (new StringField('continue_url_template', 'continueUrlTemplate'))->addFlags(new ApiAware()),
            (new JsonField('platform_allowlist', 'platformAllowlist'))->addFlags(new ApiAware()),
            (new JsonField('discovery_budget', 'discoveryBudget'))->addFlags(new ApiAware()),
            ...
            (new StringField('signature_policy', 'signaturePolicy'))->addFlags(new ApiAware(), new Required()),
            // Required Idempotency-Key for non-idempotent UCP operations.
            (new BoolField('idempotency_required', 'idempotencyRequired'))->addFlags(new ApiAware(), new Required()),
            new CustomFields(),
