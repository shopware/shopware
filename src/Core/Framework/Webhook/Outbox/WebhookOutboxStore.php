...
class WebhookOutboxStore
{
    ...
    public function resumeDeliveriesForWebhook(string $webhookId): void
    {
        ...
        RetryableTransaction::retryable($this->connection, function () use ($webhookId, $now, $nowFormatted): void {
            ...
            if ($state !== false && $state !== EndpointState::Healthy->value) {
                return;
            }
    ...
    /**
     * Cancels expired holds, then releases the oldest remaining row as the only trial. Runs in the
     * caller's transaction.
     */
    public function releaseOneTrial(string $webhookId): bool
    {
        $this->cancelAgedHeldRows($webhookId);
    ...
    /**
     * Cancels the webhook's held rows older than the grace window, in the caller's transaction.
     */
