...
class WebhookDeliveryService
{
    ...
    {
        $classification = ErrorClassification::fromStatusCode($result->statusCode ?? 0);
        ...
        $state = $this->webhookHealthService->recordFailure($webhookId, $classification, $entry->executionCount, $entry);
        if ($state === null) {
            $this->logLeaseLost($webhookId, $entry);
