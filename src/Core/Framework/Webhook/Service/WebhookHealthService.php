...
class WebhookHealthService
{
    ...
    public function recordSuccess(string $webhookId): void
    {
        ...
        $this->transition($webhookId, fn (HealthRow $row): ?HealthRow => $this->decideSuccess($row), trigger: WebhookActivationTrigger::Trial);
    }
    ...
    public function recordFailure(string $webhookId, ErrorClassification $classification, int $attempt): EndpointState
    {
        ...
        $change = $this->transition($webhookId, fn (HealthRow $row): ?HealthRow => $this->decideFailure($row, $classification, $attempt), cause: match ($classification) {
    ...
    public function reactivate(string $webhookId, WebhookActivationTrigger $trigger): bool
    {
        ...
        return $change !== null && $change->changedState();
    }
    ...
    /**
     * The dedicated action ($fromAnyState) carries operator intent in every state and turns an escalation
     * disable into an operator kill; a mirrored active=false flip carries intent only where it changes the value.
     */
    public function disableByOperator(string $webhookId, bool $fromAnyState): bool
    {
    ...
    private function runDueReleases(): void
    {
        ...
        foreach ($candidates as $webhookId) {
            ...
            }, trigger: WebhookActivationTrigger::Idle);
        }
    }
    ...
    {
        // suspended_since reads the pre-update cursor, so it must be assigned before updated_at.
        $this->connection->executeStatement(
            \sprintf(
                'UPDATE webhook_health
                 SET suspended_since = TIMESTAMPADD(MICROSECOND, TIMESTAMPDIFF(MICROSECOND, updated_at, :now), suspended_since),
                     updated_at = :now
                 WHERE endpoint_state = :suspended
                   AND suspended_since IS NOT NULL
                   AND updated_at IS NOT NULL AND updated_at < :now
                   AND webhook_id IN (SELECT w.id FROM webhook w %s)',
                $appId === null ? 'JOIN app a ON a.id = w.app_id WHERE a.active = 0' : 'WHERE w.app_id = :appId'
            ),
            [
     ...
     * columns, flip the backlog and emit the lifecycle event. $decide returns null to leave the row untouched.
     *
     * @param \Closure(HealthRow): ?HealthRow $decide
     * @param WebhookActivationTrigger|null $trigger required when $decide can return a HEALTHY row
     * @param SuspensionCause|null $cause required when $decide can return a SUSPENDED row
     *
     * @return HealthChange|null the rows before and after, or null when the webhook no longer exists
     */
    private function transition(string $webhookId, \Closure $decide, ?WebhookActivationTrigger $trigger = null, ?SuspensionCause $cause = null): ?HealthChange
    {
        $id = Uuid::fromHexToBytes($webhookId);
        ...
        $this->emit($webhookId, $change, $trigger, $cause);
     ...
     */
    private function emit(string $webhookId, HealthChange $change, ?WebhookActivationTrigger $trigger, ?SuspensionCause $cause): void
    {
