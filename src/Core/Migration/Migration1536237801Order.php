    ...
    public function update(Connection $connection): void
    {
        $connection->executeQuery('
            CREATE TABLE `order` (
              ...
              `price` json NOT NULL,
              `amount_total` double GENERATED ALWAYS AS (JSON_UNQUOTE(price->"$.totalPrice")) VIRTUAL,
              `amount_net` double GENERATED ALWAYS AS (JSON_UNQUOTE(price->"$.netPrice")) VIRTUAL,
              `position_price` double GENERATED ALWAYS AS (JSON_UNQUOTE(price->"$.positionPrice")) VIRTUAL,
              `tax_status` varchar(255) GENERATED ALWAYS AS (JSON_UNQUOTE(price->"$.taxStatus")) VIRTUAL,
              `shipping_costs` json NOT NULL,
              `shipping_total` double GENERATED ALWAYS AS (JSON_UNQUOTE(price->"$.totalPrice")) VIRTUAL,
