...
class Migration1556447212RefactorSearchKeywords extends MigrationStep
{
    ...
    {
        $connection->executeUpdate('ALTER TABLE `product` ADD `searchKeywords` binary(16) NULL;');

        $connection->executeUpdate('DROP TABLE IF EXISTS search_document;');

        $connection->executeUpdate('DROP TABLE IF EXISTS search_dictionary;');
