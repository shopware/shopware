        ...
        $query
            ...
            BEGIN
                IF NEW.price != OLD.price AND (NEW.currency_price = OLD.currency_price OR (NEW.currency_price IS NULL AND OLD.currency_price IS NULL)) THEN
                    SET NEW.currency_price = `{
                        CONCAT("c", HEX(NEW.currency_id)): {
                            "currencyId": HEX(NEW.currency_id),
                            "net": NEW.price,
                            "gross": NEW.price,
                            "linked": false
                        }
                ...
                ELSEIF NEW.price = OLD.price AND NEW.currency_id = OLD.currency_id AND NEW.currency_price != OLD.currency_price THEN
                    SET NEW.price = JSON_EXTRACT(
                        JSON_EXTRACT(NEW.currency_price, "$.*.gross"),
                        "$[0]"
                    );
                    SET NEW.currency_id = UNHEX(JSON_EXTRACT(
                        JSON_EXTRACT(NEW.currency_price, "$.*.currencyId"),
                        "$[0]"
                    ));
                END IF;
            END;';
        ...
        $query
            ...
            BEGIN
                ...
                ELSEIF NEW.price IS NULL AND NEW.currency_id IS NULL AND NEW.currency_price IS NOT NULL THEN
                    ...
                    SET NEW.currency_id = JSON_EXTRACT(
                        JSON_EXTRACT(NEW.currency_price, "$.*.currencyId"),
                        "$[0]"
                    );
                END IF;
