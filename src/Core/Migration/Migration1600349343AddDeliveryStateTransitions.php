...
class Migration1600349343AddDeliveryStateTransitions extends MigrationStep
{
    ...
    public function update(Connection $connection): void
    {
        ...
        $missingStates = $this->fetchMissingOrderDeliveryStates($connection, $stateMachineId, $openStateId);
    ...
    {
        return $connection->fetchColumn("SELECT id FROM state_machine WHERE technical_name = 'order_delivery.state'");
    }

    private function fetchOpenOrderDeliveryStateId(Connection $connection): string
    {
        return $connection->fetchColumn("SELECT initial_state_id FROM state_machine WHERE technical_name = 'order_delivery.state'");
    }

    private function fetchMissingOrderDeliveryStates(Connection $connection, string $stateMachineId, string $openStateId): array
    {
