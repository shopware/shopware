...
class Migration1607581275AddProductSearchConfigurationTest extends TestCase
{
    ...
    private function rollbackLanguage(): void
    {
        $enENLocaleId = $this->getLocaleId('en-GB');
        $this->connection->update(
            'language',
            [
                'name' => self::ENGLISH_LANGUAGE_NAME,
                'locale_id' => $enENLocaleId,
                'translation_code_id' => $enENLocaleId,
            ],
            ['id' => Uuid::fromHexToBytes(Defaults::LANGUAGE_SYSTEM)]
        );
        $deLocaleId = $this->getLocaleId('de-DE');
        $this->connection->update(
            'language',
            [
                'name' => self::GERMAN_LANGUAGE_NAME,
