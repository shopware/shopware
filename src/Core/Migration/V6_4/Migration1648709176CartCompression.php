...
class Migration1648709176CartCompression extends MigrationStep
{
    ...
    {
        if (!DbTableHelper::columnExists($connection, 'cart', 'compressed')) {
        ...
        if (!DbTableHelper::columnExists($connection, 'cart', 'payload')) {
            $connection->executeStatement('ALTER TABLE `cart` ADD `payload` LONGBLOB NULL;');
        }

        if (!DbTableHelper::columnExists($connection, 'cart', 'cart')) {
            return;
        }
