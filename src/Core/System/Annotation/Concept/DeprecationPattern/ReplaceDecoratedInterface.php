...
// The ReplaceDecoratedInterface pattern is used every time we have to change an interface, that is designed to be used with service decoration.
...
// To be able to do the change anyway we introduce a new Interface with the new public API and deprecate the old Interface to be removed in the next eligible minor version.
// We update our service to also implement the new Interface in addition to the deprecated one.
// If necessary and possible we refactor the service so that it shares most of the logic for both interface implementations
// and make it as easy as possible to later remove the implemented methods of the deprecated Interface.
//
// In the callers of the service we have to remove the type declaration in the constructor and document that the Service can either be of the new or the deprecated interface.
// When the service is called we have to do an instanceof check on the service to determine if the injected service implements the new or just the deprecated interface.
// If it implements the new Interface we call the interface in the new way, if it doesn't implement the new Interface it is problably a decoration from a plugin that has not yet adapted to the Interface change we therefore call then the deprecated Interface.
