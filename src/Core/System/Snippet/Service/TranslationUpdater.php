...
readonly class TranslationUpdater
{
     ...
     */
    public function planOfflineInstall(array $locales): TranslationInstallPlan
    {
        $localesToLink = array_values(array_filter(
            $locales,
            fn (string $locale) => $this->translationLoader->hasTranslationFiles($locale),
        ...
        return new TranslationInstallPlan(
            localesToLink: $localesToLink,
            unavailableLocales: array_values(array_diff($locales, $localesToLink)),
        );
    }
    ...
    public function install(
        ...
        return new TranslationUpdateResult($plan->localesToDownload, $plan->localesToLink);
    }
     ...
     * an update, then stores the collection as the new local metadata. Refreshing never changes whether a language is
     * active, and it never creates one that is not installed — that is what install() is for.
     */
    private function update(MetadataCollection $metadata, Context $context): TranslationUpdateResult
    {
        $localesRequiringUpdate = $metadata->getLocalesRequiringUpdate();
