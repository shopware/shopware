...
class CampaignsLinksResource extends Resource
{
    ...
    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Framework\Event\CampaignsLinksWrittenEvent
    {
        $event = new \Shopware\Framework\Event\CampaignsLinksWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Framework\Write\Resource\CampaignsLinksResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\CampaignsLinksResource::createWrittenEvent($updates));
        }
