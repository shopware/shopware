...
class EmotionCategoriesResource extends Resource
{
    ...
    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Framework\Event\EmotionCategoriesWrittenEvent
    {
        $event = new \Shopware\Framework\Event\EmotionCategoriesWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Framework\Write\Resource\EmotionCategoriesResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\EmotionCategoriesResource::createWrittenEvent($updates));
        }
