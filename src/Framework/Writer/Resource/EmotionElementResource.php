...
class EmotionElementResource extends Resource
{
    ...
    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Framework\Event\EmotionElementWrittenEvent
    {
        $event = new \Shopware\Framework\Event\EmotionElementWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Framework\Write\Resource\EmotionElementResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\EmotionElementResource::createWrittenEvent($updates));
        }
