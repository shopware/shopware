...
class ListingFacetTranslationResource extends Resource
{
    ...
    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Framework\Event\ListingFacetTranslationWrittenEvent
    {
        $event = new \Shopware\Framework\Event\ListingFacetTranslationWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Framework\Write\Resource\ListingFacetResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\ListingFacetResource::createWrittenEvent($updates));
        }
        ...
        if (!empty($updates[\Shopware\Framework\Write\Resource\ListingFacetTranslationResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\ListingFacetTranslationResource::createWrittenEvent($updates));
        }
