...
class ListingFacetTranslationWriteResource extends WriteResource
{
    ...
    public static function createWrittenEvent(array $updates, TranslationContext $context, array $errors = []): ListingFacetTranslationWrittenEvent
    {
        $event = new ListingFacetTranslationWrittenEvent($updates[self::class] ?? [], $context, $errors);

        unset($updates[self::class]);

        if (!empty($updates[ListingFacetWriteResource::class])) {
            $event->addEvent(ListingFacetWriteResource::createWrittenEvent($updates, $context));
        }
