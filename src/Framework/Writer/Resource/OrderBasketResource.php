...
class OrderBasketResource extends Resource
{
    ...
    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Framework\Event\OrderBasketWrittenEvent
    {
        $event = new \Shopware\Framework\Event\OrderBasketWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Framework\Write\Resource\OrderBasketResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\OrderBasketResource::createWrittenEvent($updates));
        }
