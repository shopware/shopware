...
class OrderNotesResource extends Resource
{
    ...
    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Framework\Event\OrderNotesWrittenEvent
    {
        $event = new \Shopware\Framework\Event\OrderNotesWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Framework\Write\Resource\OrderNotesResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\OrderNotesResource::createWrittenEvent($updates));
        }
