...
class OrderResource extends Resource
{
    ...
    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Framework\Event\OrderWrittenEvent
    {
        $event = new \Shopware\Framework\Event\OrderWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Framework\Write\Resource\OrderResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\OrderResource::createWrittenEvent($updates));
        }
