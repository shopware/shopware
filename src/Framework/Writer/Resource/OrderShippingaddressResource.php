...
class OrderShippingaddressResource extends Resource
{
    ...
    public function __construct()
    {
        ...
        $this->fields[self::COUNTRYID_FIELD] = (new IntField('countryID'))->setFlags(new Required());
    ...
    {
        return [
            \Shopware\Framework\Write\Resource\OrderShippingaddressResource::class,
        ];
    }

    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Framework\Event\OrderShippingaddressWrittenEvent
    {
        $event = new \Shopware\Framework\Event\OrderShippingaddressWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Framework\Write\Resource\OrderShippingaddressResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\OrderShippingaddressResource::createWrittenEvent($updates));
        }
