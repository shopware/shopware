...
class StatisticAddressPoolResource extends Resource
{
    ...
    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Framework\Event\StatisticAddressPoolWrittenEvent
    {
        $event = new \Shopware\Framework\Event\StatisticAddressPoolWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Framework\Write\Resource\StatisticAddressPoolResource::class])) {
            $event->addEvent(\Shopware\Framework\Write\Resource\StatisticAddressPoolResource::createWrittenEvent($updates));
        }
