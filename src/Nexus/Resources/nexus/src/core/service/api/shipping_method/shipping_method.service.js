export default function ShippingMethodService(client) {
    /**
    ...
    /**
     * @param {Number} limit - The limit of shippingMethods you want to receive
     * @param {Number} offset - Offset of the shippingMethods you want to receive
     * @returns {Promise}
     */
    function readAll(limit = 25, offset = 0) {
        return client.get(`/shippingMethod.${returnFormat}?limit=${limit}&offset=${offset}`).then((response) => {
            return response.data;
        });
    }

    /**
     * @param {String} uuid - ShippingMethod UUID
     * @returns {Promise}
     */
    function readByUuid(uuid) {
        ...
        return client.get(`/shippingMethod/${uuid}.${returnFormat}`).then((response) => {
            return response.data;
        });
    }

    /**
     * Updates a single shippingMethod. Partial updates are supported using the {@param payload}.
     *
     * @param {String} uuid - ShippingMethod UUID
    ...
    function updateByUuid(uuid, payload) {
        ...
        return client.patch(`/shippingMethod/${uuid}.json`, payload).then((response) => {
            return response.data;
        });
    }
