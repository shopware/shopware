...
class PaymentMethodBasicReader
{
    ...
    public function __construct(Connection $connection, PaymentMethodBasicHydrator $hydrator)
    {
    ...
    {
        $query = new PaymentMethodBasicQuery($this->connection, $context);

        $query->andWhere('paymentMethod.uuid IN (:uuids)');
