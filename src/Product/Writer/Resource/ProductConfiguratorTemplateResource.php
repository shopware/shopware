...
class ProductConfiguratorTemplateResource extends Resource
{
    ...
    public static function createWrittenEvent(array $updates, array $errors = []): \Shopware\Product\Event\ProductConfiguratorTemplateWrittenEvent
    {
        $event = new \Shopware\Product\Event\ProductConfiguratorTemplateWrittenEvent($updates[self::class] ?? [], $errors);

        unset($updates[self::class]);

        if (!empty($updates[\Shopware\Product\Writer\Resource\ProductConfiguratorTemplateResource::class])) {
            $event->addEvent(\Shopware\Product\Writer\Resource\ProductConfiguratorTemplateResource::createWrittenEvent($updates));
        }
