...
describe('Admin & Storefront: test customer group registration', () => {
    ...
    it('@package: should register with new customer group', () => {
        ...
        cy.get('.alert-content').contains('VIP Customers');

        // Accept customer in admin
        cy.visit(`${Cypress.env('admin')}#/sw/customer/index`);
        cy.get('.sw-customer-list__requested-group-label .sw-label__caption').contains('Verzoek VIP Customer');
        cy.clickContextMenuItem(
            '.sw-customer-list__view-action',
            '.sw-context-button__button',
            `.sw-data-grid__row--0`
        );
        cy.get('.sw-alert__message').contains('VIP Customers').should('be.visible');
        ...
        cy.get('[class="sw-data-grid__row sw-data-grid__row--0"] .sw-label--appearance-pill').contains('VIP Customer');
    ...
    it('@package: should register with new commercial customer group', () => {
        ...
        cy.wait('@registerCustomer').its('response.statusCode').should('equal', 302);
        cy.url().should('include', 'account');
        cy.get('.alert-content').contains('VIP Commercial');

        // Accept commercial customer in admin
        cy.visit(`${Cypress.env('admin')}#/sw/customer/index`);
        cy.get('.sw-customer-list__requested-group-label .sw-label__caption').contains('Verzoek VIP Commercial');
        cy.clickContextMenuItem(
            '.sw-customer-list__view-action',
            '.sw-context-button__button',
            `.sw-data-grid__row--0`
        );
        cy.get('.sw-alert__message').contains('VIP Commercial').should('be.visible');
        cy.get('.sw-button__content').contains('Akkoord').click();
        cy.awaitAndCheckNotification('De klantgroep is verwijderd');
        cy.get('.smart-bar__back-btn').click();
        cy.get('[class="sw-data-grid__row sw-data-grid__row--0"] .sw-label--appearance-pill').contains('VIP Commercial');
