...
namespace Shopware\Shop\Searcher\Handler;
...
class ActiveHandler implements HandlerInterface, AggregatorInterface
{
    ...
    public function handle(
        ...
        if ($criteriaPart instanceof ActiveSorting) {
            $builder->addOrderBy('shop.active', $criteriaPart->getDirection());

            return;
        }

        /* @var ActiveCondition $criteriaPart */
        $builder->andWhere('shop.active = :active_condition');
    ...
    public function aggregate(
        ...
        $builder->groupBy('shop.active');

        $builder->select(['shop.active', 'COUNT(shop.uuid) as item_count']);
