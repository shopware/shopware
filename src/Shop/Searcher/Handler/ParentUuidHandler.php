...
class ParentUuidHandler implements HandlerInterface, AggregatorInterface
{
    ...
    public function handle(
        ...
        if ($criteriaPart instanceof ParentUuidSorting) {
            $builder->addOrderBy('shop.parent_uuid', $criteriaPart->getDirection());

            return;
        }

        /* @var ParentUuidCondition $criteriaPart */
        $builder->andWhere('shop.parent_uuid IN (:parent_uuid_condition)');
    ...
    public function aggregate(
        ...
        $builder->select(['DISTINCT shop.parent_uuid']);
