...
async function main() {
    ...
    await Promise.all(bundlesWithComponents.map(async ([bundleName, config]) => {
        ...
        const customConfigTs = path.join(storefrontAppDir, 'vite.components.config.ts');
        const customConfig = fs.existsSync(customConfigMts)
            ? customConfigMts
            : (fs.existsSync(customConfigTs) ? customConfigTs : null);
        ...
        if (customConfig !== null) {
            if (customConfig === customConfigTs) {
                console.warn('  Using legacy custom config "vite.components.config.ts". Prefer ".mts" for ESM consistency.');
            }
