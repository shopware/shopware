 ...
 */
export const componentRoot = path.join(projectRoot, 'src/Storefront/Resources/views/components');
...
/**
 * @deprecated Use {@link buildComponentStyleEntries} which also handles plain
 *             `.css` entries via the virtual-module shim pipeline. Kept for
 *             backward compatibility during the transition.
 */
export async function buildComponentScssEntries(): Promise<Record<string, string>> {
    const { scssEntries } = await buildComponentStyleEntries();
    return scssEntries;
}
