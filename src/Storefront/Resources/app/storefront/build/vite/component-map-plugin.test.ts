...
async function build(fixtureRoot: string, fixture: Fixture): Promise<BuildResult> {
    ...
    const vendorMapAsset = assets.find(asset => asset.fileName === '.vite/vendor-map.json');
    const vendorMap: Record<string, string> = vendorMapAsset
        ? JSON.parse(String(vendorMapAsset.source))
        : {};
