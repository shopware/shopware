...
describe('Shopware runtime component lifecycle', () => {
    ...
    it('blocks cross-origin component imports resolved from import maps', async () => {
        const importMapScript = document.createElement('script');
        importMapScript.type = 'importmap';
        importMapScript.textContent = JSON.stringify({
            imports: {
                'Sw:Blocked': 'https://evil.example/blocked-component.js',
            },
        });
        document.body.appendChild(importMapScript);

        const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
        const component = await Shopware.getComponent('Sw:Blocked');

        expect(component).toBeUndefined();
        expect(errorSpy).toHaveBeenCalledOnce();
        expect(errorSpy).toHaveBeenCalledWith('Blocked component import for Sw:Blocked: https://evil.example/blocked-component.js');
    });

    it('blocks unsafe protocol component imports', async () => {
        const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
        const component = await Shopware.getComponent('javascript:alert(1)');

        expect(component).toBeUndefined();
        expect(errorSpy).toHaveBeenCalledOnce();
        expect(errorSpy).toHaveBeenCalledWith('Blocked component import for javascript:alert(1): javascript:alert(1)');
    });
