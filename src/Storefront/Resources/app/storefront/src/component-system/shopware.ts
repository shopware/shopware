...
class Shopware extends EventEmitter {
    ...
    public async getComponent(componentName: string | null | undefined): Promise<typeof ShopwareComponent | undefined> {
        ...
        if (!this.isAllowedComponentSpecifier(componentSpecifier)) {
            console.error(`Blocked component import for ${componentName}: ${componentSpecifier}`);
    ...
    private isAllowedComponentSpecifier(componentSpecifier: string): boolean {
        try {
            const url = new URL(componentSpecifier, window.location.origin);
            ...
            // In Vite dev-server mode, component entries are imported from /@fs/*
            // and can be served from a loopback host (e.g. localhost:5175).
            const isLoopbackHost = url.hostname === 'localhost'
                || url.hostname === '127.0.0.1'
                || url.hostname === '::1'
                || url.hostname === '[::1]';

            if (isLoopbackHost && url.pathname.startsWith('/@fs/')) {
                return true;
            }
