...
describe('src/plugin/cms-gdpr-video-element/cms-gdpr-video-element.plugin', () => {
    ...
    afterEach(() => {
        jest.clearAllMocks();
        ['youtube-video', 'vimeo-video'].forEach((cookieName) => CookieStorageHelper.removeItem(cookieName));
    ...
    test('should not replace the video a second time when the element is already detached', () => {
        const videoUrl = 'https://www.youtube.com/embed/test';
        const iframeTitle = 'Test Video';
        const options = { videoUrl, iframeTitle, iframeClasses: [] };
        ...
        expect(cmsGdprVideoElement._replaceElementWithVideo()).toBe(true);

        // The consent events can fire again after the element was replaced, e.g. when the
        // cookie offcanvas is saved afterwards. The element is detached by then.
        expect(cmsGdprVideoElement._replaceElementWithVideo()).toBe(false);
