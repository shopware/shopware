...
export default defineConfig(async ({ command }): Promise<UserConfig> => {
    ...
    return {
        // Public URL base for built assets. Component chunks are published into
        // `public/components/` by ComponentPublisher, so every chunk is served
        // at `/components/<chunkPath>`. Rolldown's preload-helper uses this
        ...
        //
        // The manifest.json `file` entries stay relative to `outDir`, so the
        // PHP ComponentPublisher continues to work without changes. Static
