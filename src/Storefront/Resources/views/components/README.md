...
1. Vite builds component JS/TS/SCSS/CSS from `Resources/views/components/` into `Resources/app/storefront/dist-es/components/`.
2. `storefront:publish-components` copies those emitted files to `public/components/` and updates `var/cache/component-manifest.json`.
3. Theme compile reads that manifest and injects component imports/styles at runtime.
