...
class ThemeRuntimeConfigStorage
{
    ...
    public function save(ThemeRuntimeConfig $config): void
    {
        $this->connection->executeStatement(<<<'SQL'
            REPLACE INTO `theme_runtime_config` (theme_id, technical_name, resolved_config, view_inheritance, script_files, icon_sets, component_import_map, updated_at)
            VALUES (:themeId, :technicalName, :resolvedConfig, :viewInheritance, :scriptFiles, :iconSets, :componentImportMap, :updatedAt)
            SQL, [
            ...
            'componentImportMap' => $config->componentImportMap !== null
                ? json_encode($config->componentImportMap, \JSON_THROW_ON_ERROR)
                : null,
    ...
    private function hydrateRecord(array $record): ThemeRuntimeConfig
    {
        return ThemeRuntimeConfig::fromArray([
            ...
            'componentImportMap' => isset($record['component_import_map'])
                ? json_decode($record['component_import_map'], true, 512, \JSON_THROW_ON_ERROR)
                : null,
