...
export const CreateCustomField = base.extend<{ CreateCustomField: Task }, FixtureTypes>({
    CreateCustomField: async ({ AdminCustomFieldDetail, InstanceMeta }, use) => {
        const task = (customFieldName: string, customFieldType: 'Text field' | 'number') => {
            return async function CreateCustomField() {
                ...
                if (satisfies(InstanceMeta.version, '<6.7')) {
                    ...
                    await (await AdminCustomFieldDetail.getSelectFieldListitem(AdminCustomFieldDetail.customFieldTypeSelectionList, customFieldType)).click();
                }
