...
test(
    'New customers can register as commercial customers in the Storefront.',
    { tag: ['@Registration', '@Storefront'] },
    async ({ ShopCustomer, StorefrontAccountLogin, StorefrontAccount, IdProvider, Register, TestDataService }) => {
...
test(
    'New customers cannot register as commercial customers without a VAT Reg.No.',
    { tag: ['@Registration', '@Storefront'] },
    async ({ ShopCustomer, StorefrontAccountLogin, Register, TestDataService, DefaultSalesChannel }) => {
        await TestDataService.setSystemConfig({ 'core.loginRegistration.showAccountTypeSelection': true });
        ...
        await test.step('Verify registration is blocked due to missing VAT Reg.No.', async () => {
            ...
            await ShopCustomer.expects(StorefrontAccountLogin.vatRegNoInput).toHaveCSS(
                'border-color',
                'rgb(194, 0, 23)'
            );
        });
...
test(
    'New customers cannot register as commercial customers with an invalid VAT Reg.No.',
    { tag: ['@Registration', '@Storefront'] },
    async ({ ShopCustomer, StorefrontAccountLogin, IdProvider, Register, TestDataService, DefaultSalesChannel }) => {
        await TestDataService.setSystemConfig({ 'core.loginRegistration.showAccountTypeSelection': true });
        ...
        await test.step('Verify registration is blocked due to invalid VAT Reg.No. format', async () => {
            ...
            await ShopCustomer.expects(StorefrontAccountLogin.page.locator('.invalid-feedback')).toContainText(
                'The VAT Reg.No. you have entered does not have the correct format.'
            );
        });
    }
);
