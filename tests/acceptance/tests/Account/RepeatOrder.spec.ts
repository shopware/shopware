...
test(
    'As a customer, I want to repeat a previous order via the storefront account.',
    {
        tag: [
            '@Order',
            '@Account',
    ...
    async ({ ShopCustomer, StorefrontAccountOrder, StorefrontOffCanvasCart, TestDataService, Login }) => {
        const product = await TestDataService.createBasicProduct();
        const customer = await TestDataService.createCustomer();
        const productQuantity = 5;
        const order = await TestDataService.createOrder([{ product: product, quantity: productQuantity }], customer);

        // create another order with different quantity to ensure that the correct order is repeated
        await TestDataService.createOrder([{ product: product, quantity: 1 }], customer);
