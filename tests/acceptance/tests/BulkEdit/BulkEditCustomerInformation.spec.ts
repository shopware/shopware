...
test(
    'As a merchant, I can perform bulk edits on customer information',
    { tag: '@BulkEdits' },
    async ({
        TestDataService,
        ShopAdmin,
        ...
        await test.step('Verifies the changes applied for bulk edited customers', async () => {
            for (const customer of [
                ...
                //Verify custom field
                const customFieldSetTabContent =
                    await AdminCustomerDetail.getCustomFieldSetCardContentByName(customFieldSetName);
