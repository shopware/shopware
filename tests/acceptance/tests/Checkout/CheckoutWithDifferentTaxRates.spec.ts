...
test(
    ...
    async ({
        ...
        await test.step('Add 2 identical products to chart, proceed to checkout and validate on confirm page the tax price.', async () => {
            ...
            await ShopCustomer.expects(StorefrontCheckoutConfirm.taxPrice).toContainText(`${currencyIcon}3.19`);
        });
        ...
        await test.step('Submit the order, navigate to checkout finish page and validate the tax price.', async () => {
            ...
            await ShopCustomer.expects(StorefrontCheckoutFinish.taxPrice).toContainText(`${currencyIcon}3.19`);
        });
        ...
        await test.step('Navigate to customers account order page and validate the orders tax price.', async () => {
            ...
            await ShopCustomer.expects(orderLocators.taxPrice).toContainText(`${currencyIcon}3.19`);
        });
    }
);
...
test(
    ...
    async ({
        ...
        await test.step('Add 2 different products with same tax rate to chart, proceed to checkout and validate on confirm page the tax price.', async () => {
            ...
            await ShopCustomer.attemptsTo(ProceedFromProductToCheckout());
            await ShopCustomer.expects(StorefrontCheckoutConfirm.taxPrice).toContainText(`${currencyIcon}1.30`);
        });
        ...
        await test.step('Submit the order, navigate to checkout finish page and validate the tax price.', async () => {
            ...
            await ShopCustomer.expects(StorefrontCheckoutFinish.taxPrice).toContainText(`${currencyIcon}1.30`);
        });
        ...
        await test.step('Navigate to customers account order page and validate the orders tax price.', async () => {
            ...
            const orderLocators = await StorefrontAccountOrder.getOrderByOrderNumber(orderNumber);
            await orderLocators.orderDetailButton.click();
            await ShopCustomer.expects(orderLocators.taxPrice).toContainText(`${currencyIcon}1.30`);
        });
    }
);
...
test(
    ...
    async ({
        ...
        await test.step('Add 2 different products with different tax rates to chart, proceed to checkout and validate on confirm page the tax price.', async () => {
            ...
            await ShopCustomer.attemptsTo(ProceedFromProductToCheckout());
            await ShopCustomer.expects(StorefrontCheckoutConfirm.taxPrice.nth(0)).toContainText(`${currencyIcon}3.19`);
            await ShopCustomer.expects(StorefrontCheckoutConfirm.taxPrice.nth(1)).toContainText(`${currencyIcon}1.31`);
        });
        ...
        await test.step('Submit the order, navigate to checkout finish page and validate the tax price.', async () => {
            ...
            await ShopCustomer.expects(StorefrontCheckoutFinish.taxPrice.nth(0)).toContainText(`${currencyIcon}3.19`);
            await ShopCustomer.expects(StorefrontCheckoutFinish.taxPrice.nth(1)).toContainText(`${currencyIcon}1.31`);
        });
        ...
        await test.step('Navigate to customers account order page and validate the orders tax price.', async () => {
            ...
            const orderLocators = await StorefrontAccountOrder.getOrderByOrderNumber(orderNumber);
            await orderLocators.orderDetailButton.click();
            await ShopCustomer.expects(orderLocators.taxPrice.nth(0)).toContainText(`${currencyIcon}3.19`);
            await ShopCustomer.expects(orderLocators.taxPrice.nth(1)).toContainText(`${currencyIcon}1.31`);
        });
    }
);
...
test(
    ...
    async ({
        ...
        await test.step('Add one product with tax rate to chart, proceed to checkout and validate on confirm page the tax price.', async () => {
            ...
            await ShopCustomer.attemptsTo(ProceedFromProductToCheckout());
            await ShopCustomer.expects(StorefrontCheckoutConfirm.taxPrice).toContainText(`${currencyIcon}0.00`);
        });
        ...
        await test.step('Submit the order, navigate to checkout finish page and validate the tax price.', async () => {
            ...
            await ShopCustomer.expects(StorefrontCheckoutFinish.taxPrice).toContainText(`${currencyIcon}0.00`);
        });
        ...
        await test.step('Navigate to customers account order page and validate the orders tax price.', async () => {
            ...
            const orderLocators = await StorefrontAccountOrder.getOrderByOrderNumber(orderNumber);
            await orderLocators.orderDetailButton.click();
            await ShopCustomer.expects(orderLocators.taxPrice).toContainText(`${currencyIcon}0.00`);
        });
    }
);
