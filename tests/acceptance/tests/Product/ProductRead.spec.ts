...
test(
    ...
    async ({
        ...
        await test.step("Read the product data from the product listing", async () => {
            ...
            await ShopAdmin.expects(productRow.productNumber).toHaveText(
                product.productNumber,
            );
            await ShopAdmin.expects(productRow.productManufacturer).toHaveText(
                manufacturer.name,
            );
        ...
        await test.step("Read the measurements from the specifications tab", async () => {
            ...
            await ShopAdmin.expects(AdminProductDetail.widthInput).toHaveValue(
                "60",
            );
            await ShopAdmin.expects(AdminProductDetail.heightInput).toHaveValue(
                "30",
            );
            await ShopAdmin.expects(AdminProductDetail.lengthInput).toHaveValue(
                "120",
            );
            await ShopAdmin.expects(AdminProductDetail.weightInput).toHaveValue(
                "5",
            );
        });
    },
