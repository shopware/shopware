...
test(
    ...
    async ({ ShopCustomer, TestDataService, StorefrontProductDetail }) => {
        ...
        await ShopCustomer.expects(
            StorefrontProductDetail.productReviewRating,
        ).toBeVisible();
        await ShopCustomer.expects(
...
test(
    ...
    async ({
        ...
        await test.step("Login for review writing and validate the review form.", async () => {
            ...
            await ShopCustomer.expects(
                StorefrontProductDetail.reviewLoginForm,
            ).toBeVisible();
            await ShopCustomer.expects(
                StorefrontProductDetail.forgottenPasswordLink,
            ).toBeVisible();
            ...
            await ShopCustomer.expects(
                StorefrontProductDetail.reviewForm,
            ).toBeVisible();
            await ShopCustomer.expects(
                StorefrontProductDetail.reviewRatingPoints,
            ).toHaveCount(5);

            const starRatingPoints =
            ...
            for (let i = 0; i < starRatingPoints; i++) {
                ...
                await ShopCustomer.expects(async () => {
                    await ShopCustomer.expects(
                        StorefrontProductDetail.reviewRatingText.nth(
                            starRatingPoints - (i + 1),
                        ),
        ...
        await test.step("Create a review and validate the submitted review.", async () => {
            ...
            await ShopCustomer.fillsIn(
                StorefrontProductDetail.reviewTitleInput,
                reviewContent.title,
            );
            await ShopCustomer.fillsIn(
                StorefrontProductDetail.reviewReviewTextInput,
                reviewContent.content,
            );
            await ShopCustomer.presses(
                StorefrontProductDetail.reviewSubmitButton,
            );
            ...
            await ShopCustomer.expects(
                StorefrontProductDetail.reviewItemRatingPoints,
            ).toHaveCount(4);
            await ShopCustomer.expects(
                StorefrontProductDetail.reviewItemTitle,
            ).toHaveText(reviewContent.title);
            await ShopCustomer.expects(
                StorefrontProductDetail.reviewItemContent,
            ).toHaveText(reviewContent.content);
