...
test.describe('Product Analytics - Consent Modal Validation', { tag: '@ProductAnalytics' }, () => {
    test(
    ...
    test(
        'Only authorized users in administration can change store consent and user data consent',
        { tag: '@ProductAnalytics' },
        async ({ TestDataService, SalesChannelBaseConfig, browser, InstanceMeta }) => {
            ...
            await test.step('Setup user which can not change store consent but user data consent', async () => {
                ...
                const onlyChangeUserProfilePermissions = await TestDataService.createAclRole({
