...
test(
    'As a shop customer, I want to accept Google Analytics tracking via the basic cookie consent banner in the storefront.',
    { tag: ['@Settings', '@Storefront'] },
    async ({ ShopCustomer, StorefrontHome, TestDataService, DefaultSalesChannel }) => {
        await test.step('Configure sales channel analytics and verify cookie banner visibility on the home page', async () => {
            ...
            await TestDataService.assignSalesChannelAnalytics(
                DefaultSalesChannel.salesChannel.id,
                salesChannelAnalytics.id
            );
        ...
        await test.step('Enable Google Analytics and marketing tracking via cookie settings', async () => {
            ...
            await ShopCustomer.presses(StorefrontHome.consentDialogStatisticsCheckbox);
            await ShopCustomer.presses(StorefrontHome.consentDialogMarketingdCheckbox);
