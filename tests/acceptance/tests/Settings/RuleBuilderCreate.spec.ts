...
test('As an admin user, I want to create a rule', { tag: '@Rule' }, async ({
    ...
    await test.step('Validate rule via UI', async () => {
        ...
        await ShopAdmin.expects(AdminRuleDetail.conditionDateRangeDateFieldFirst).toHaveValue(testConfig.fromDate.replace('+00:00', ''));
        await ShopAdmin.expects(AdminRuleDetail.conditionDateRangeDateFieldSecond).toHaveValue(testConfig.toDate.replace('+00:00', ''));
...
function formatDateOnlyWithOffset(date) {
