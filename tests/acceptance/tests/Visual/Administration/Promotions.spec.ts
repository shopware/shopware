...
test(
    'Visual: Promotions Listing Page',
    { tag: '@Visual' },
    async ({ ShopAdmin, TestDataService, AdminPromotionsListing }) => {
...
test(
    'Visual: Promotion Detail Page',
    { tag: '@Visual' },
    async ({ ShopAdmin, TestDataService, AdminPromotionCreate, AdminPromotionDetail }) => {
        ...
        const testPromo = await TestDataService.createPromotionWithCode({
            name: 'TestPromo',
            code: '123',
            discounts: [{ type: 'percentage', value: 15, scope: 'cart', considerAdvancedRules: true }],
        });
