...
test(`Test visibility of reviews`, async ({ salesChannelProduct, storefrontPage, anonStorefrontPage, adminPage }) => {
    ...
    await storefrontPage
        .getByPlaceholder("Enter your review...")
        .fill("This review has no meaning whatsoever\n\nlorem ipsum\n\nasdf");
    await storefrontPage.getByRole("button", { name: "Submit" }).click();

    // verify review is visible
    await expect(storefrontPage.getByText("My test review")).toBeVisible();
    ...
    await expect(anonStorefrontPage.getByRole("alert").locator("div").first()).toHaveText(
    ...
    await anonStorefrontPage.getByRole("tab", { name: "Reviews" }).click();
    await expect(anonStorefrontPage.getByText("My test review")).toBeVisible();
