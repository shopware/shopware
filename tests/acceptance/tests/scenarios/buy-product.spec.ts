...
for (let i = 0; i < 48; ++i) {
    test(`Buy product from storefront ${i}`, async ({
        ...
        await storefrontPage.getByRole("button", { name: "Add to shopping cart" }).click();
        ...
        await expect(
            storefrontPage.getByLabel("I have read and accepted the general terms and conditions.")
        ).toBeChecked();
        ...
        const locator = storefrontPage.getByText(orderNumberRegex);

        const orderNumberText = await locator.textContent();
        const [, orderNumber] = orderNumberText.match(orderNumberRegex);
        ...
        */
        ...
        await adminPage.getByPlaceholder("Search all orders...").fill(orderNumber);

        await expect(adminPage.locator(".sw-search-bar__footer")).toBeHidden();
        await adminPage.getByRole("link", { name: orderNumber }).click();

        await expect(adminPage.getByRole("heading")).toHaveText(`Order ${orderNumber}`);

        await storefrontPage.pause();
        await expect(adminPage.getByText(defaultStorefront.customer.email)).toBeVisible();

        await expect(adminPage.locator('dt:has-text("Total including VAT") + dd')).toHaveText("€10.00");
    });
}
