...
/**
 * POC smoke: drives DocumentGenerator end-to-end with the tc-lib-pdf-based PdfRenderer
 * and writes the resulting PDF to <project_dir>/var/poc/ so it can be opened manually.
 *
 * Run with:
 *   vendor/bin/phpunit tests/integration/Core/Checkout/DocumentV2/Renderer/TcLibPdfPocTest.php
 *
 * @internal
 */
#[Package('after-sales')]
#[Group('poc')]
class TcLibPdfPocTest extends TestCase
{
    use DocumentV2Trait;
    ...
    public function testWritesInvoicePdfToDisk(): void
    {
        ...
        static::assertStringStartsWith('%PDF-', $bytes, 'output is not a valid PDF');

        $projectDir = static::getContainer()->getParameter('kernel.project_dir');
        $outDir = $projectDir . '/var/poc';
        if (!\is_dir($outDir)) {
            \mkdir($outDir, 0o755, true);
        }
        $path = $outDir . '/invoice-' . \date('Ymd-His') . '.pdf';
        \file_put_contents($path, $bytes);

        static::assertFileExists($path);

        \fwrite(\STDERR, \PHP_EOL . 'POC PDF written to: ' . $path . \PHP_EOL);
    }
