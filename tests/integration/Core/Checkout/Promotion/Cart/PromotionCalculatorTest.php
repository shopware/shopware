...
class PromotionCalculatorTest extends TestCase
{
    ...
    public function testPromotionSetGroupIsNotEligible(): void
    {
        ...
        $this->createSetGroupDiscount($promotionId, 1, static::getContainer(), 100, null, applierKey: '3');
        $this->createSetGroupFixture('COUNT', 3, 'PRICE_ASC', $promotionId, static::getContainer());

        $promotion = new LineItem($promotionId, PromotionProcessor::LINE_ITEM_TYPE, 'Black Friday');
        $promotion->setPriceDefinition(new PercentagePriceDefinition(100));
        ...
        $lineItem3 = new LineItem($product3, LineItem::PRODUCT_LINE_ITEM_TYPE, $product3);
        ...
        static::assertSame(30.0, $firstCalculatedCard->getPrice()->getTotalPrice());

        $secondCalculatedCard = $cartService->remove($cart, $lineItem->getId(), $context);
        static::assertNotNull($secondCalculatedCard->getErrors()->first());
        static::assertSame('Promotion Black Friday not eligible for cart!', $secondCalculatedCard->getErrors()->first()->getMessage());
        static::assertCount(2, $secondCalculatedCard->getLineItems());
    }
