...
class ImportExportV2ActionControllerTest extends TestCase
{
    ...
    public function testStartImportEndpointQueuesRunAndProcessesImportedProduct(): void
    {
        ...
        static::assertNotEmpty($run['inputFileId']);

        $browser->request('GET', '/api/_action/import-export-v2/file/' . $run['inputFileId']);
        static::assertSame(Response::HTTP_OK, $browser->getResponse()->getStatusCode());
        $file = $this->decodeBrowserJson($browser)['file'];
        static::assertSame('products.json', $file['name']);
    ...
    public function testStartExportEndpointQueuesRunAndDownloadReturnsExportedFile(): void
    {
        ...
        static::assertSame('completed', $run['state']);
        static::assertNotEmpty($run['outputFileId']);

        $browser->request('GET', '/api/_action/import-export-v2/file/' . $run['outputFileId'] . '/download');
        static::assertSame(Response::HTTP_OK, $browser->getResponse()->getStatusCode());
    ...
    public function testStartExportEndpointProcessesMultipleChunks(): void
    {
        $taxId = $this->createTax('Export tax', 19.0);
        $productOne = $this->createProduct('SW22001', 'Export one', $taxId, 11, true);
        $productTwo = $this->createProduct('SW22002', 'Export two', $taxId, 12, true);
        $productThree = $this->createProduct('SW22003', 'Export three', $taxId, 13, true);

        $browser = $this->createClient();
        $browser->jsonRequest('POST', '/api/_action/import-export-v2/export', [
            'profileName' => 'product-json',
            'chunkSize' => 2,
            'recordIds' => [$productOne, $productTwo, $productThree],
        ]);
    ...
    public function testStartExportEndpointSupportsFiltersWithoutRecordIds(): void
    {
        $taxId = $this->createTax('Filtered export tax', 19.0);
        $this->createProduct('SW24001', 'Filtered one', $taxId, 11, true);
        $this->createProduct('SW24002', 'Filtered two', $taxId, 12, true);
        $this->createProduct('SW24003', 'Filtered out', $taxId, 13, false);
        ...
        static::assertSame(Response::HTTP_ACCEPTED, $browser->getResponse()->getStatusCode());
        $run = $this->decodeBrowserJson($browser)['run'];
        static::assertSame([], $run['recordIds']);
        static::assertCount(1, $run['exportFilters']);

        $this->runWorker();

        $browser->request('GET', '/api/_action/import-export-v2/run/' . $run['id']);
        static::assertSame(Response::HTTP_OK, $browser->getResponse()->getStatusCode());
        $completedRun = $this->decodeBrowserJson($browser)['run'];
        static::assertSame('completed', $completedRun['state']);
        static::assertSame(2, $completedRun['totalRecords']);

        $browser->request('GET', '/api/_action/import-export-v2/file/' . $completedRun['outputFileId'] . '/download');
        static::assertSame(Response::HTTP_OK, $browser->getResponse()->getStatusCode());
        ...
        static::assertNotContains('SW24003', array_column(array_column($payload, 'identifier'), 'productNumber'));
    }

    public function testStartExportEndpointRejectsMixedRecordIdsAndFilters(): void
    {
        $browser = $this->createClient();
        $browser->jsonRequest('POST', '/api/_action/import-export-v2/export', [
            'profileName' => 'product-json',
            'recordIds' => [Uuid::randomHex()],
            'filters' => [[
                'type' => 'equals',
                'field' => 'active',
                'value' => true,
