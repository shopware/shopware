...
class WebhookDispatchGateTest extends TestCase
{
    ...
    public function testAuthStreakSuspensionLeavesSameAppSiblingHealthyAndDelivering(): void
    {
        ...
        $this->insertClaimableRow('evt-trial', 'wh-broken');
        $entry = $this->claimRunningEntry('evt-trial');

        $health->recordFailure($this->ids->get('wh-broken'), ErrorClassification::NonTransientAuth, 1, $entry);
        $health->recordFailure($this->ids->get('wh-broken'), ErrorClassification::NonTransientAuth, 1, $entry);
        $health->recordFailure($this->ids->get('wh-broken'), ErrorClassification::NonTransientAuth, 1, $entry);
        ...
        // The trial attempt that drove the suspension is over, so the count below sees only what the dispatch wrote.
        $this->deleteOutboxRows('evt-trial');
    ...
    public function testClaimableEscapeRowIsDeliveredOnceAndItsFailureFeedsTheStreak(): void
    {
        ...
        $entry = $store->markRunning($this->ids->get('evt-escape'));
        static::assertNotNull($entry);
        static::assertSame([], $store->fetchDue($partitionKey, $claimable, 10));

        $health = static::getContainer()->get(WebhookHealthService::class);
        $result = $health->recordFailure($this->ids->get('wh-escape'), ErrorClassification::NonTransientAuth, 1, $entry);
    ...
    {
        $entry = static::getContainer()->get(WebhookOutboxStore::class)->markRunning($this->ids->get($eventKey));
        static::assertNotNull($entry, 'the seeded row must be claimable for the worker');

        return $entry;
    }

    private function deleteOutboxRows(string $eventKey): void
    {
