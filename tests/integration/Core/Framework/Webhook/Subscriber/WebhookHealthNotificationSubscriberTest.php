...
/**
 * Locks down the Admin trip/recovery notifications (ADR §Lifecycle events & trip notification).
 * One notification per suspension episode, keyed on the unchanged suspended_since — a flapping
 * endpoint never re-alarms within one episode, because alarm fatigue teaches operators to ignore
 * the channel. DISABLED always notifies, and a recovery notice closes the loop.
 *
 * @internal
 */
#[Package('framework')]
class WebhookHealthNotificationSubscriberTest extends TestCase
{
    ...
    public function testSuspensionNotifiesExactlyOncePerEpisode(): void
    {
        ...
        Feature::withFeatureEnabled('WEBHOOKS_REWORK', function () use ($since): void {
            // A re-trip within the same episode carries the SAME suspended_since (set-once) —
            // it must not alarm again.
    ...
    public function testANewEpisodeAfterRecoveryNotifiesAgain(): void
    {
        Feature::withFeatureEnabled('WEBHOOKS_REWORK', function (): void {
            ...
            // Recovery cleared suspended_since; the next trip starts a NEW episode.
    ...
    public function testDisabledAlwaysNotifiesAndNamesTheOrigin(): void
    {
        ...
        static::assertSame(2, $this->notificationCount(), 'entering DISABLED always notifies');
    ...
    {
        Feature::withFeatureEnabled('WEBHOOKS_REWORK', function (): void {
            // An idle promotion or a plain DEGRADED recovery never crossed SUSPENDED — no notice.
            $this->dispatcher->dispatch(new WebhookActivatedEvent(
                $this->ids->get('wh'),
                null,
                EndpointState::Degraded,
