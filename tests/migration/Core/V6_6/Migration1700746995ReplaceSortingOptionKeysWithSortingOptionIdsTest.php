...
class Migration1700746995ReplaceSortingOptionKeysWithSortingOptionIdsTest extends TestCase
{
    ...
    public function testMigration(): void
    {
        ...
        $categoryConfigs = $this->connection->fetchAllAssociative(
            <<<SQL
                SELECT slot_config FROM category_translation
                WHERE category_id = :categoryId
                  AND category_version_id = :categoryVersionId
                  AND (language_id = :langId0 OR language_id = :langId1);
            SQL,
            [
        ...
        $categoryConfig0 = json_decode($categoryConfigs[0]['slot_config'], true);
        ...
        $categoryConfig1 = json_decode($categoryConfigs[1]['slot_config'], true);
        static::assertSame(
            $sortingIds['name-asc'],
    ...
    public function testMigrationForCorruptedEntries(): void
    {
        ...
        static::assertSame(
            'bvhlbjkbk',
            $categoryConfig1[Uuid::fromBytesToHex($productListingSlot)]['defaultSorting']['value']
        );
