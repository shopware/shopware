...
class Migration1773112488SetDefaultCmsPageIdForCategoriesTest extends TestCase
{
    ...
    public function testMigrationSetsCmsPageIdForCategoriesWithoutOne(): void
    {
        $defaultCmsPageId = $this->getDefaultCmsPageId();
        if ($defaultCmsPageId === null) {
            static::markTestSkipped('No default CMS page configured');
        }
        ...
        static::assertSame($defaultCmsPageId, $result);
    }
    ...
    public function testMigrationDoesNotOverwriteExistingCmsPageId(): void
    {
        ...
        if ($existingCmsPageId === null) {
            static::markTestSkipped('No CMS page available');
        }
    ...
    public function testMigrationDoesNothingWhenNoDefaultConfigured(): void
    {
        $originalConfig = $this->connection->fetchAssociative(
        ...
        $result = $this->connection->fetchOne(
            'SELECT `cms_page_id` FROM `category` WHERE `id` = :id',
            ['id' => $categoryId]
        );
