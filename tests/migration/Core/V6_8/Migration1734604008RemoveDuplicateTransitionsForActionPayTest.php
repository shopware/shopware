...
use Shopware\Core\Migration\V6_8\Migration1734604008RemoveDuplicateTransitionsForActionPay;

/**
 * @internal
 */
#[Package('checkout')]
#[CoversClass(Migration1734604008RemoveDuplicateTransitionsForActionPay::class)]
class Migration1734604008RemoveDuplicateTransitionsForActionPayTest extends TestCase
{
    ...
    {
        $migration = new Migration1734604008RemoveDuplicateTransitionsForActionPay();
        static::assertSame(1734604008, $migration->getCreationTimestamp());
    }

    public function testMigrationRemovesDuplicateTransitions(): void
    {
        ...
        $this->connection->executeStatement('INSERT INTO `state_machine_transition` (id, state_machine_id, from_state_id, to_state_id, action_name, created_at) VALUES (?, ?, ?, ?, ?, ?)', [$duplicateTransition2, $stateMachineId, $fromStateId, $toStateId, 'paid', (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT)]);

        // Insert a valid transition that shouldn't be removed
