...
class CartRuleLoaderTest extends TestCase
{
    ...
    public function testFallsBackToOriginalCartErrorsWhenProcessedCartHasNone(): void
    {
        $reason = 'rule not matching or inactive';
        $customer = new CustomerEntity();
        $customer->setId('test-id');
        ...
        $cart = new Cart('test');
        $cart->getErrors()->add(new ShippingMethodBlockedError(
            id: 'shipping-method-id',
            name: 'Standard',
            reason: $reason,
        ));
        ...
        static::assertSame(
            'Die Versandart "Standard" ist für Ihren aktuellen Warenkorb gesperrt.',
            $cart->getErrors()->first()?->getTranslatedMessage(),
        );
    }
}
