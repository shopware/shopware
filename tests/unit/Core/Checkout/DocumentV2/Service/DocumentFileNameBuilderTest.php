...
class DocumentFileNameBuilderTest extends TestCase
{
    ...
    public function testBuildUsesTheConfiguredFileNamePrefixVerbatimForMultipleDocuments(): void
    {
        $documents = [
            $this->createDocument(config: ['filenamePrefix' => 'Rechnung-', 'documentNumber' => '10000']),
            $this->createDocument(config: ['filenamePrefix' => 'Rechnung-', 'documentNumber' => '10001']),
        ];

        static::assertSame('Rechnung-' . self::DOWNLOAD_DATE, $this->build($documents));
    }
