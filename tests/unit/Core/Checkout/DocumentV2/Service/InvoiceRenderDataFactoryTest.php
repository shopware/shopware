 ...
 */
#[Package('after-sales')]
#[CoversClass(InvoiceRenderDataFactory::class)]
class InvoiceRenderDataFactoryTest extends TestCase
{
    ...
    public function testEnrichOrderCriteria(): void
    {
        ...
        $this->createFactory()->enrichOrderCriteria($criteria);
    ...
    public function testBuildResolvesIntraCommunityDelivery(
        array $config,
        OrderEntity $order,
        ...
        $result = $factory->build($order, $this->buildRequest($order), Context::createDefaultContext());
    ...
    public function testBuildDerivesPaymentDueDateFromDocumentDate(): void
    {
        $factory = $this->createFactory(['paymentDueDate' => '+30 days']);
    ...
    public function testBuildReturnsNullPaymentDueDateForInvalidDocumentDate(): void
    {
        $factory = $this->createFactory(['paymentDueDate' => '+30 days']);

        $order = self::createOrder();
        $request = $this->buildRequest($order, documentDate: 'not-a-date');

        $result = $factory->build($order, $request, Context::createDefaultContext());

        static::assertNull($result->paymentDueDate);
    }

    public function testBuildThrowsWhenDocumentNumberMissing(): void
    {
        $factory = $this->createFactory();
        $order = self::createOrder();
        ...
        $factory->build($order, $request, Context::createDefaultContext());
    }

    public function testBuildResolvesDeliveryDateFromDeliveriesWhenV68IsInactive(): void
    {
    ...
    public function testBuildConvertsMutableShippingDateToImmutable(): void
    {
        $factory = $this->createFactory();
        $order = self::createOrder(
            country: self::createCountry(companyTaxEnabled: true, isEu: true),
    ...
    private function buildRequest(OrderEntity $order, string $documentDate = '2026-05-05T12:00:00+00:00'): DocumentGenerationRequest
    {
