...
/**
 * Cobertura for {@see VariantListingUpdater} is collected from the unit testsuite only; the migration
 * job restricts coverage sources to migration paths. Behaviour is also asserted in integration tests.
 *
 * @internal
 */
...
class VariantListingUpdaterTest extends TestCase
{
    ...
    public function testUpdateAppliesSha256DisplayGroupForListingGroupConfiguration(): void
    {
        ...
        $this->connection->insert('product', [
            ...
            'product_number' => 'vl-unit-codecov-parent',
            'stock' => 10,
            'variant_listing_config' => $listingConfig,
            'display_group' => null,
        ]);
        foreach ([[$variantAId, 'vl-unit-codecov-a'], [$variantBId, 'vl-unit-codecov-b']] as [$variantId, $number]) {
            $this->connection->insert('product', [
                'id' => $variantId,
                'version_id' => $liveVersion,
                'parent_id' => $parentId,
        ...
        static::assertSame($expectedRed, $displayByNumber['vl-unit-codecov-a']);
        static::assertSame($expectedGreen, $displayByNumber['vl-unit-codecov-b']);
