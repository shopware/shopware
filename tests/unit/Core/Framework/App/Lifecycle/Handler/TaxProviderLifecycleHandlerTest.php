 ...
 */
#[Package('checkout')]
#[CoversClass(TaxProviderLifecycleHandler::class)]
class TaxProviderLifecycleHandlerTest extends TestCase
{
    ...
    public function testCreateNewTaxProvider(): void
    {
        ...
        $handler = new TaxProviderLifecycleHandler($repo);
    ...
    public function testSkippedWhenNoAppSecret(): void
    {
        ...
        $handler = new TaxProviderLifecycleHandler($repo);
        $handler->install($this->buildContext($manifest, 'foo', false));
    }

    private function buildContext(Manifest $manifest, string $appId, bool $hasAppSecret = true): AppPersistContext
    {
        $app = new AppEntity();
        $app->setId($appId);
        $app->setActive(true);
