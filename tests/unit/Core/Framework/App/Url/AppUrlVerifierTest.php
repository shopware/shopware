...
class AppUrlVerifierTest extends TestCase
{
    public function testVerifyReturnsTrueInDevMode(): void
    {
        ...
        $verifier = new AppUrlVerifier('dev', '6.7.1.0', $cache, $http, $lockFactory, $clock);
    ...
    public function testVerifyReturnsFalseIfnNoAppUrl(): void
    {
        ...
        $verifier = new AppUrlVerifier('prod', '6.7.1.0', $cache, $http, $lockFactory, $clock);
    ...
    public function testVerifyOutcomes(
        ...
        $verifier = new AppUrlVerifier('prod', '6.7.1.0', $cache, new MockHttpClient($responseFactory), $locks, $clock);
    ...
    public static function verifyOutcomeProvider(): iterable
    {
        ...
        yield 'invalid URL > HARD_FAIL' => [
            ...
            false,
            VerificationStatus::HARD_FAIL,
            'APP_URL is invalid: HTTPS is required.',
        ];
    }
    ...
    public function testBackoffAndRetryScenario(array $httpResponseCodes, array $steps, VerificationStatus $expectedFinal): void
    {
        ...
        $verifier = new AppUrlVerifier('prod', '6.7.1.0', $cache, $http, $locks, $clock);
        $shopId = ShopId::v2('shop-id', [AppUrl::IDENTIFIER => 'https://example.com']);
        ...
        if ($expectedFinal === VerificationStatus::HARD_FAIL) {
            ...
            static::assertEmpty($item->getMetadata()['expiry'], 'Hard-fail state should not expire.');
        }
    }
    ...
    public function testForceVerifyClearsOldStateAndRecomputes(): void
    {
        $cache = new ArrayAdapter();
        $clock = new MockClock();
        $locks = new LockFactory(new InMemoryStore());
    ...
    {
        $cache = new ArrayAdapter();
        $verifier = new AppUrlVerifier('prod', '6.7.1.0', $cache, new MockHttpClient(), new LockFactory(new InMemoryStore()), new MockClock());
