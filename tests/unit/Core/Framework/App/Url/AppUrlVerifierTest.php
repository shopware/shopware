...
class AppUrlVerifierTest extends TestCase
{
    public function testVerifyReturnsFalseIfnNoAppUrl(): void
    {
        ...
        $verifier = new AppUrlVerifier($cache, $http, $lockFactory, $clock);

        $shopId = $this->createMock(ShopId::class);
        $shopId->method('getFingerprint')
            ->with(AppUrl::IDENTIFIER)
            ->willReturn(null);
    ...
    public function testVerifyOutcomes(
        ...
        $verifier = new AppUrlVerifier($cache, new MockHttpClient($responseFactory), $locks, $clock);
    ...
    public function testBackoffAndRetryScenario(array $httpResponseCodes, array $steps, VerificationStatus $expectedFinal): void
    {
        ...
        $verifier = new AppUrlVerifier($cache, $http, $locks, $clock);
