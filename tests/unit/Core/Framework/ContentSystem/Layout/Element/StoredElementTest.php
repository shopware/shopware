...
{
    /**
     * PHP casts every numeric array key to an integer, so a numeric string and the bare integer land on the
     * same runtime key; each label names the form a caller would have written.
     *
    ...
    public function testConstructorRejectsANumericPropertyKey(string $key): void
    {
        $this->expectExceptionObject(ContentSystemException::invalidMapKey('Element property map', 'int'));

        new StoredElement('element-1', 'core:text', properties: [$key => StoredValue::ofString('value')]);
    }

    #[TestDox('rejects a numeric data requirement key')]
    #[DataProvider('numericWiringKeyProvider')]
    public function testConstructorRejectsANumericDataRequirementKey(string $key): void
    {
        $this->expectExceptionObject(ContentSystemException::invalidMapKey('Element data requirement map', 'int'));

        new StoredElement('element-1', 'core:text', dataRequirements: [
            $key => new DataRequirement('product', 'entity', new StubLoaderConfig()),
        ]);
    }

    #[TestDox('rejects a numeric slot key')]
    #[DataProvider('numericWiringKeyProvider')]
    public function testConstructorRejectsANumericSlotKey(string $key): void
    {
        $this->expectExceptionObject(ContentSystemException::invalidMapKey('Element slot map', 'int'));

        new StoredElement('element-1', 'core:section', slots: [
            $key => [StoredElementBuilder::create('core:text', 'child-1')->build()],
        ]);
    }
