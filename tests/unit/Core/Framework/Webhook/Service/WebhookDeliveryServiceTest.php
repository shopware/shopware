...
class WebhookDeliveryServiceTest extends TestCase
{
    ...
    public function testAnExceptionCarryingA2xxIsASuccess(): void
    {
        $message = $this->createMessage();
    ...
    public function testCurrentTransientFailureHoldsADegradedDelivery(): void
    {
        ...
        $this->webhookHealthService->expects($this->once())->method('recordFailure')
            ->with($message->getWebhookId(), ErrorClassification::TransientServer, 1, $entry)
    ...
    {
        [$message, $entry] = $this->prepareDelivery(new Response(500, [], '{"error":"fail"}'), 2);

        $this->webhookOutboxStore->method('markPendingRetry')->willReturn(true);

        $this->webhookHealthService->expects($this->never())->method('recordLegacyFailure');
        $this->webhookHealthService->expects($this->once())->method('recordFailure')
            ->with($message->getWebhookId(), ErrorClassification::TransientServer, 2, $entry)
    ...
    {
        [$message, $entry] = $this->prepareDelivery(new Response(302, [], ''));

        // Without a Location header, Guzzle returns the 302 instead of following it.
        $this->webhookHealthService->expects($this->once())->method('recordFailure')
            ->with($message->getWebhookId(), ErrorClassification::TransientRedirect, 1, $entry)
            ->willReturn(EndpointState::Healthy);
        $this->webhookHealthService->expects($this->never())->method('recordSuccess');
    ...
    public function testAStaleFailureLeavesTheDeliveryRowUntouched(): void
    {
        [$message, $entry] = $this->prepareDelivery(new Response(503));
    ...
    public function testFailedDeliveryRowPlacementFollowsClassificationAndResultingState(
        ...
        [$message, $entry] = $this->prepareDelivery(new Response($responseStatus, [], '{"error":"fail"}'), $executionCount);

        $this->webhookHealthService->expects($this->once())->method('recordFailure')
            ->with($message->getWebhookId(), $classification, $executionCount, $entry)
