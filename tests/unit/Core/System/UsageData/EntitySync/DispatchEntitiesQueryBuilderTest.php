...
class DispatchEntitiesQueryBuilderTest extends TestCase
{
    ...
    public function testWithRunDateConstraintCreatedOperation(): void
    {
        ...
        static::assertSame(
            $this->queryHelper,
            $this->queryHelper->withLastCollectionAllowedDateConstraint($message, $runDate),
        );

        static::assertEquals(
            CompositeExpression::or(
                '`updated_at` IS NULL',
                '`updated_at` <= :lastCollectionAllowedDate',
            ),
            QueryBuilderDataExtractor::getWhere($this->queryHelper->getQueryBuilder()),
        );
        ...
        $parameters = $this->queryHelper->getQueryBuilder()->getParameters();
        static::assertCount(1, $parameters);
        static::assertArrayHasKey('lastCollectionAllowedDate', $parameters);
        static::assertSame($runDate->format(Defaults::STORAGE_DATE_TIME_FORMAT), $parameters['lastCollectionAllowedDate']);
    }
