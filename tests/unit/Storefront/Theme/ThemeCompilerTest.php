...
class ThemeCompilerTest extends TestCase
{
    ...
    private ComponentPublisher&MockObject $componentPublisher;
    ...
    protected function setUp(): void
    {
        ...
        $this->componentPublisher = $this->createMock(ComponentPublisher::class);
        $this->componentPublisher->method('readComponentManifest')->willReturn([]);
        $this->componentPublisher->method('buildExtensionVendorScopes')->willReturn([]);
    ...
    public function testVariableEnrichmentEventCanModifyVariables(): void
    {
        ...
        static::assertIsString($capturedScss);
    ...
    public function testBuildComponentImportMapReturnsBaseStructureWithShopwareEntry(): void
    {
        ...
        static::assertArrayNotHasKey('styles', $result);
    }

    public function testBuildComponentImportMapIncludesComponentEntriesFromManifest(): void
    {
        $this->localFilesystem = $this->createMock(LocalFilesystem::class);
        $this->localFilesystem->method('exists')->willReturnCallback(
            static fn (string $path): bool => str_ends_with($path, 'shopware.js')
        ...
        $this->componentPublisher = $this->createMock(ComponentPublisher::class);
        $this->componentPublisher->method('readComponentManifest')->willReturn([
            'Sw:Button' => ['js' => '/components/Sw/Button-abc.js'],
            'MyExt:Card' => [
                'js' => '/components/MyExt/Card-def.js',
                'css' => ['/components/MyExt/Card-ghi.css'],
            ],
        ]);
        $this->componentPublisher->method('buildExtensionVendorScopes')->willReturn([]);
        ...
        static::assertNotNull($result);
        static::assertSame('/components/Sw/Button-abc.js', $result['imports']['Sw:Button']);
        static::assertSame('/components/MyExt/Card-def.js', $result['imports']['MyExt:Card']);
        static::assertArrayHasKey('styles', $result);
        static::assertSame(['/components/MyExt/Card-ghi.css'], $result['styles']);
    }

    public function testBuildComponentImportMapExpandsMultipleCssFilesPerComponent(): void
    {
        // A component manifest entry may carry several stylesheets (sibling
        // .scss + JS-imported vendor CSS, multiple side-effect imports, etc.).
        // Each must produce its own entry in the styles list, in source order.
        $this->localFilesystem = $this->createMock(LocalFilesystem::class);
        $this->localFilesystem->method('exists')->willReturnCallback(
            static fn (string $path): bool => str_ends_with($path, 'shopware.js')
        );
        ...
        $this->componentPublisher->method('readComponentManifest')->willReturn([
            'MyExt:Slider' => [
                'js' => '/components/MyExt/Slider-abc.js',
                'css' => [
                    '/components/MyExt/Slider-own.css',
                    '/components/MyExt/Slider-vendor.css',
                ],
            ],
        ]);
        $this->componentPublisher->method('buildExtensionVendorScopes')->willReturn([]);
    ...
    public function testBuildComponentImportMapOmitsStylesKeyWhenNoComponentCssPresent(): void
    {
        $this->localFilesystem = $this->createMock(LocalFilesystem::class);
        $this->localFilesystem->method('exists')->willReturnCallback(
            static fn (string $path): bool => str_ends_with($path, 'shopware.js')
    ...
    public function testBuildComponentImportMapIncludesExtensionVendorScopes(): void
    {
        $this->localFilesystem = $this->createMock(LocalFilesystem::class);
        $this->localFilesystem->method('exists')->willReturnCallback(
            static fn (string $path): bool => str_ends_with($path, 'shopware.js')
        ...
        $scopes = [
            'http://localhost/components/MyExt/' => [
                'debounce' => 'http://localhost/components/MyExt/vendor/debounce-abc.js',
            ],
        ];

        $this->componentPublisher = $this->createMock(ComponentPublisher::class);
        $this->componentPublisher->method('readComponentManifest')->willReturn([]);
        $this->componentPublisher->method('buildExtensionVendorScopes')->willReturn($scopes);

        $result = $this->createThemeCompiler()->buildComponentImportMap(TestDefaults::SALES_CHANNEL, 'theme-id');

        static::assertNotNull($result);
        static::assertArrayHasKey('scopes', $result);
        static::assertSame($scopes, $result['scopes']);
    }

    public function testBuildComponentImportMapReadsCoreVendorMapFromStorefrontJsDir(): void
    {
        // Core vendor chunks emitted by componentMapPlugin become top-level imports
        // pointing at public/components/ URLs.
        $vendorMap = ['debounce' => 'Sw/vendor/debounce-abc.js'];

        $this->localFilesystem = $this->createMock(LocalFilesystem::class);
        $this->localFilesystem->method('exists')->willReturnCallback(
            static fn (string $path): bool => str_ends_with($path, 'shopware.js')
                || str_ends_with($path, 'vendor-map.json')
        );
        $this->localFilesystem
            ->method('readFile')
            ->willReturn((string) json_encode($vendorMap));
        ...
        static::assertNotNull($result);
        static::assertSame('/components/Sw/vendor/debounce-abc.js', $result['imports']['debounce']);
    }

    public function testBuildComponentImportMapPassesConfiguredPublicBaseUrlToVendorScopes(): void
    {
        $this->localFilesystem = $this->createMock(LocalFilesystem::class);
        $this->localFilesystem->method('exists')->willReturnCallback(
            static fn (string $path): bool => str_ends_with($path, 'shopware.js')
        ...
        $this->componentPublisher
            ->expects($this->once())
            ->method('buildExtensionVendorScopes')
            ->with('')
            ->willReturn([]);

        $this->createThemeCompiler()->buildComponentImportMap(TestDefaults::SALES_CHANNEL, 'theme-id');
    }
